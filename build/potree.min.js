...
this.version){b=c;break}if(null!==b){var c=b.getBoundingBox().clone().applyMatrix4(this.matrix),e=c.getSize(new THREE.Vector3),d=this.expandAndFindByBox(c,b.getLevel());b.demVersion=this.version;Potree.getDEMWorkerInstance().onmessage=(a)=>{a=new Float32Array(a.data.dem.data);for(let b of d){let d=b.box.getSize(new THREE.Vector3);for(let f=0;f<this.tileSize;f++)for(let g=0;g<this.tileSize;g++){let h=this.tileSize*(b.box.min.x+f/(this.tileSize-1)*d.x-c.min.x)/e.x,k=this.tileSize*(b.box.min.y+g/(this.tileSize-
1)*d.y-c.min.y)/e.y;0>h||h>this.tileSize||0>k||k>this.tileSize||(h=Math.min(Math.floor(h),this.tileSize-1),k=Math.min(Math.floor(k),this.tileSize-1),b.data[f+this.tileSize*g]=a[h+this.tileSize*k])}b.createMipMap();b.mipMapNeedsUpdate=!0;Potree.getDEMWorkerInstance().working=!1}};a=b.geometryNode.geometry.attributes.position.array;b={boundingBox:{min:b.getBoundingBox().min.toArray(),max:b.getBoundingBox().max.toArray()},position:(new Float32Array(a)).buffer};a=[b.position];Potree.getDEMWorkerInstance().working=
