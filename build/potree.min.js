...
extensions.get('WEBGL_depth_texture'),null!==c)?c.UNSIGNED_INT_24_8_WEBGL:0};Potree.attributeLocations={position:0,color:1,intensity:2,classification:3,returnNumber:4,numberOfReturns:5,pointSourceID:6,indices:7,normal:8,spacing:9};class EnumItem{constructor(a){for(let b of Object.keys(a))this[b]=a[b]}inspect(){return`Enum(${this.name}: ${this.value})`}}
...
x.slice(z*y,z*y+y),D.set(B,v*y),B.length===y)v++;else break;console.log(`width: ${b}`);console.log(`height: ${m}`);console.log(`numStrips: ${l}`);console.log('stripByteCounts',A.join(', '));console.log('stripOffsets',t.join(', '));a=new k;a.width=b;a.height=m;a.buffer=D;a.metadata=c;return a}}class m{static toTiffBuffer(a,b={}){let c=new Uint8Array([73,73,42,0,8,0,0,0]),[e,g]=[a.width,a.height];var k=[new h(f.IMAGE_WIDTH,d.SHORT,1,null,e),new h(f.IMAGE_HEIGHT,d.SHORT,1,null,g),new h(f.BITS_PER_SAMPLE,
...
l;a.Exporter=m;return a}({});Potree.version={major:1,minor:6,suffix:'-nogui'};
...
THREE.ShaderChunk.logdepthbuf_fragment+'\n\n\t#if defined(weighted_splats)\n\t\tfloat distance = 2.0 * length(gl_PointCoord.xy - 0.5);\n\t\tfloat weight = max(0.0, 1.0 - distance);\n\t\tweight = pow(weight, 1.5);\n\n\t\tgl_FragColor.a = weight;\n\t\tgl_FragColor.xyz = gl_FragColor.xyz * weight;\n\t#endif\n}';Potree.PointCloudMaterial=class extends THREE.RawShaderMaterial{constructor(a={}){super();this.visibleNodesTexture=Potree.utils.generateDataTexture(2048,1,new THREE.Color(16777215));this.visibleNodesTexture.minFilter=THREE.NearestFilter;this.visibleNodesTexture.magFilter=THREE.NearestFilter;let b=function(a,b){return void 0!==a?a:b};var c=b(a.size,1);let e=b(a.minSize,2),d=b(a.maxSize,50);a=b(a.treeType,Potree.TreeType.OCTREE);this._pointSizeType=Potree.PointSizeType.FIXED;this._shape=Potree.PointShape.SQUARE;
this._pointColorType=Potree.PointColorType.RGB;this._weighted=this._useClipBox=!1;this._gradient=Potree.Gradients.SPECTRAL;this._treeType=a;this._snapEnabled=this._useEDL=!1;this._numSnapshots=0;this._defaultElevationRangeChanged=this._defaultIntensityRangeChanged=!1;this.clipBoxes=[];this.clipPolygons=[];this.gradientTexture=Potree.PointCloudMaterial.generateGradientTexture(this._gradient);this.fog=this.lights=!1;this.defines=new Map;this.attributes={position:{type:'fv',value:[]},color:{type:'fv',
...
value:[]},uSnapViewInv:{type:'Matrix4fv',value:[]},uShadowColor:{type:'3fv',value:[0,0,0]}};this.classification=Potree.Classification.DEFAULT;this.defaultAttributeValues.normal=[0,0,0];this.defaultAttributeValues.classification=[0,0,0];this.defaultAttributeValues.indices=[0,0,0,0];this.vertexColors=THREE.VertexColors;c=this.getDefines();this.vertexShader=c+Potree.Shaders['pointcloud.vs'];this.fragmentShader=c+Potree.Shaders['pointcloud.fs']}setDefine(a,b){void 0!==b&&null!==b?this.defines.get(a)!==
b&&(this.defines.set(a,b),this.updateShaderSource()):this.removeDefine(a)}removeDefine(a){this.defines.delete(a)}updateShaderSource(){var a=this.getDefines();this.vertexShader=a+Potree.Shaders['pointcloud.vs'];this.fragmentShader=a+Potree.Shaders['pointcloud.fs'];this.depthFunc=THREE.LessEqualDepth;this.needsUpdate=this.depthWrite=this.depthTest=!0}onBeforeCompile(a,b){b.capabilities.logarithmicDepthBuffer&&(a.fragmentShader='#define USE_LOGDEPTHBUF\n#define USE_LOGDEPTHBUF_EXT\n#define EPSILON 1e-6\n'+
a.fragmentShader,a.vertexShader='#define USE_LOGDEPTHBUF\n#define USE_LOGDEPTHBUF_EXT\n#define EPSILON 1e-6\n'+a.vertexShader)}getDefines(){let a=[];this.pointSizeType===Potree.PointSizeType.FIXED?a.push('#define fixed_point_size'):this.pointSizeType===Potree.PointSizeType.ATTENUATED?a.push('#define attenuated_point_size'):this.pointSizeType===Potree.PointSizeType.ADAPTIVE&&a.push('#define adaptive_point_size');this.shape===Potree.PointShape.SQUARE?a.push('#define square_point_shape'):this.shape===
Potree.PointShape.CIRCLE?a.push('#define circle_point_shape'):this.shape===Potree.PointShape.PARABOLOID&&a.push('#define paraboloid_point_shape');this._useEDL&&a.push('#define use_edl');this._snapEnabled&&a.push('#define snap_enabled');this._pointColorType===Potree.PointColorType.RGB?a.push('#define color_type_rgb'):this._pointColorType===Potree.PointColorType.COLOR?a.push('#define color_type_color'):this._pointColorType===Potree.PointColorType.DEPTH?a.push('#define color_type_depth'):this._pointColorType===
Potree.PointColorType.HEIGHT?a.push('#define color_type_height'):this._pointColorType===Potree.PointColorType.INTENSITY?a.push('#define color_type_intensity'):this._pointColorType===Potree.PointColorType.INTENSITY_GRADIENT?a.push('#define color_type_intensity_gradient'):this._pointColorType===Potree.PointColorType.LOD?a.push('#define color_type_lod'):this._pointColorType===Potree.PointColorType.POINT_INDEX?a.push('#define color_type_point_index'):this._pointColorType===Potree.PointColorType.CLASSIFICATION?
a.push('#define color_type_classification'):this._pointColorType===Potree.PointColorType.RETURN_NUMBER?a.push('#define color_type_return_number'):this._pointColorType===Potree.PointColorType.SOURCE?a.push('#define color_type_source'):this._pointColorType===Potree.PointColorType.NORMAL?a.push('#define color_type_normal'):this._pointColorType===Potree.PointColorType.PHONG?a.push('#define color_type_phong'):this._pointColorType===Potree.PointColorType.RGB_HEIGHT?a.push('#define color_type_rgb_height'):
this._pointColorType===Potree.PointColorType.COMPOSITE&&a.push('#define color_type_composite');this._treeType===Potree.TreeType.OCTREE?a.push('#define tree_type_octree'):this._treeType===Potree.TreeType.KDTREE&&a.push('#define tree_type_kdtree');this.weighted&&a.push('#define weighted_splats');for(let [,b]of this.defines)a.push(b);return a.join('\n')}setClipBoxes(a){if(a){var b=this.clipBoxes.length!==a.length&&(0===a.length||0===this.clipBoxes.length);this.uniforms.clipBoxCount.value=this.clipBoxes.length;
this.clipBoxes=a;b&&this.updateShaderSource();this.uniforms.clipBoxes.value=new Float32Array(16*this.clipBoxes.length);for(b=0;b<this.clipBoxes.length;b++)this.uniforms.clipBoxes.value.set(a[b].inverse.elements,16*b);for(a=0;a<this.uniforms.clipBoxes.value.length;a++)Number.isNaN(this.uniforms.clipBoxes.value[a])&&(this.uniforms.clipBoxes.value[a]=Infinity)}}setClipPolygons(a,b){a&&(this.clipPolygons=a,this.clipPolygons.length!==a.length&&this.updateShaderSource())}get gradient(){return this._gradient}set gradient(a){this._gradient!==
...
var g=Potree.attributeLocations[e],h=d.normalized,k=this.types.get(d.array.constructor);a.vertexAttribPointer(g,d.itemSize,k,h,0,0);a.enableVertexAttribArray(g);c.vbos.set(e,{handle:f,name:e,count:d.count,itemSize:d.itemSize,type:b.attributes.position.array.constructor,version:0})}a.bindBuffer(a.ARRAY_BUFFER,null);a.bindVertexArray(null);return c}updateBuffer(a,b){var c=this.buffers.get(b);a.bindVertexArray(c.vao);for(var e in b.attributes){var d=b.attributes[e],f=Potree.attributeLocations[e],g=d.normalized,
h=this.types.get(d.array.constructor);if(c.vbos.has(e)){var k=c.vbos.get(e).handle;c.vbos.get(e).version=d.version}else k=a.createBuffer(),c.vbos.set(e,{handle:k,name:e,count:d.count,itemSize:d.itemSize,type:b.attributes.position.array.constructor,version:d.version});a.bindBuffer(a.ARRAY_BUFFER,k);a.bufferData(a.ARRAY_BUFFER,d.array,a.STATIC_DRAW);a.vertexAttribPointer(f,d.itemSize,h,g,0,0);a.enableVertexAttribArray(f)}a.bindBuffer(a.ARRAY_BUFFER,null);a.bindVertexArray(null)}fetchOctrees(){for(var a=
[],b=[this];0<b.length;){var c=b.pop();c instanceof Potree.PointCloudTree?a.push(c):(c=c.children.filter((a)=>a.visible),b.push(...c))}return{octrees:a}}renderNodes(a,b,c,e,d,f){a=a.context;b=b.material;d=d.matrixWorldInverse;var g=new THREE.Matrix4,h=new Float32Array(16),k;for(k of c)if(void 0===Potree.debug.allowedNodes||Potree.debug.allowedNodes.includes(k.name)){var l=k.sceneNode.matrixWorld;g.multiplyMatrices(d,l);e&&(c=e.offsets.get(k),f.setUniform1f('uVNStart',c));c=k.getLevel();f.setUniform('uDebug',
!0===k.debug);if(k instanceof Potree.PointCloudOctreeNode)var m=0===Object.keys(k.children).length;else k instanceof Potree.PointCloudArena4DNode&&(m=k.geometryNode.isLeaf);f.setUniform('uIsLeafNode',m);var n=f.uniformLocations.modelMatrix;n&&(h.set(l.elements),a.uniformMatrix4fv(n,!1,h));n=f.uniformLocations.modelViewMatrix;h.set(g.elements);a.uniformMatrix4fv(n,!1,h);if(b.clipPolygons&&0<b.clipPolygons.length){n=[];var p=[];for(r of b.clipPolygons){d=r.viewMatrix;var u=r.projMatrix.clone().multiply(d).multiply(l);
n.push(r.markers.length);p.push(u)}l=[].concat(...p.map((a)=>a.elements));u=Array(24*b.clipPolygons.length);for(p=0;p<b.clipPolygons.length;p++){var r=b.clipPolygons[p];for(var q=0;q<r.markers.length;q++)u[24*p+3*q]=r.markers[q].position.x,u[24*p+(3*q+1)]=r.markers[q].position.y,u[24*p+(3*q+2)]=r.markers[q].position.z}a.uniform1iv(f.uniformLocations['uClipPolygonVCount[0]'],n);a.uniformMatrix4fv(f.uniformLocations['uClipPolygonWVP[0]'],!1,l);a.uniform3fv(f.uniformLocations['uClipPolygonVertices[0]'],
u)}f.setUniform1f('uLevel',c);f.setUniform1f('uNodeSpacing',k.geometryNode.estimatedSpacing);f.setUniform1f('uPCIndex',p);c=k.geometryNode.geometry;l=null;if(this.buffers.has(c)){l=this.buffers.get(c);for(var C in c.attributes)c.attributes[C].version>l.vbos.get(C).version&&this.updateBuffer(a,c)}else l=this.createBuffer(a,c),this.buffers.set(c,l);a.bindVertexArray(l.vao);a.drawArrays(a.POINTS,0,l.numElements)}a.bindVertexArray(null)}renderOctree(a,b,c,e){var d=a.context,f=b.material,g=e.matrixWorldInverse,
h=e.matrixWorld,k=e.projectionMatrix,l=(new THREE.Matrix4).getInverse(k);new THREE.Matrix4;var m=null,n=0;if(0<=f.pointSizeType&&(f.pointSizeType===Potree.PointSizeType.ADAPTIVE||f.pointColorType===Potree.PointColorType.LOD)){m=b.computeVisibilityTextureData(c,e);var p=f.visibleNodesTexture;p.image.data.set(m.data);p.needsUpdate=!0}p=null;this.shaders.has(f)?(p=this.shaders.get(f),u=f.vertexShader,r=f.fragmentShader):(p=new Potree.Shader(d,'pointcloud',f.vertexShader,f.fragmentShader),this.shaders.set(f,
p));r=['#define num_shadowmaps0','#define num_snapshots'+(f.snapEnabled?f.numSnapshots:0),'#define num_clipboxes'+(f.clipBoxes&&f.clipBoxes.length?f.clipBoxes.length:0),'#define num_clipspheres0','#define num_clippolygons'+(f.clipPolygons&&f.clipPolygons.length?f.clipPolygons.length:0)].join('\n');var u=r+'\n'+f.vertexShader,r=r+'\n'+f.fragmentShader;p.update(u,r);f.needsUpdate=!1;for(var q of Object.keys(f.uniforms))if(u=f.uniforms[q],'t'==u.type&&(u=u.value))this.textures.has(u)||(r=new Potree.WebGLTexture(d,
u),this.textures.set(u,r)),this.textures.get(u).update();d.useProgram(p.program);1>f.opacity?(d.enable(d.BLEND),d.blendFunc(d.SRC_ALPHA,d.ONE),d.depthMask(!1),d.disable(d.DEPTH_TEST)):(d.disable(d.BLEND),d.depthMask(!0),d.enable(d.DEPTH_TEST));p.setUniformMatrix4('projectionMatrix',k);p.setUniformMatrix4('viewMatrix',g);p.setUniformMatrix4('uViewInv',h);p.setUniformMatrix4('uProjInv',l);g=f.screenHeight;p.setUniform1f('uScreenWidth',f.screenWidth);p.setUniform1f('uScreenHeight',g);p.setUniform1f('fov',
Math.PI*e.fov/180);p.setUniform1f('near',e.near);p.setUniform1f('far',e.far);a.capabilities.logarithmicDepthBuffer&&p.setUniform('logDepthBufFC',2/(Math.log(e.far+1)/Math.LN2));e instanceof THREE.OrthographicCamera?(p.setUniform('uUseOrthographicCamera',!0),p.setUniform('uOrthoWidth',e.right-e.left),p.setUniform('uOrthoHeight',e.top-e.bottom)):p.setUniform('uUseOrthographicCamera',!1);0===f.clipBoxes.length+f.clipPolygons.length?p.setUniform1i('clipTask',Potree.ClipTask.NONE):p.setUniform1i('clipTask',
f.clipTask);p.setUniform1i('clipMethod',f.clipMethod);f.clipBoxes&&0<f.clipBoxes.length&&d.uniformMatrix4fv(p.uniformLocations['clipBoxes[0]'],!1,f.uniforms.clipBoxes.value);p.setUniform1f('size',f.size);p.setUniform1f('maxSize',f.uniforms.maxSize.value);p.setUniform1f('minSize',f.uniforms.minSize.value);p.setUniform1f('uOctreeSpacing',f.spacing);p.setUniform('uOctreeSize',f.uniforms.octreeSize.value);p.setUniform3f('uColor',f.color.toArray());p.setUniform1f('uOpacity',f.opacity);p.setUniform2f('elevationRange',
