...
size:4},DATA_TYPE_INT64:{ordinal:8,size:8},DATA_TYPE_UINT64:{ordinal:9,size:8}},pa=0,qa;for(qa in C)C[pa]=C[qa],pa++;t.POSITION_CARTESIAN=new t(x.POSITION_CARTESIAN,C.DATA_TYPE_FLOAT,3);t.RGBA_PACKED=new t(x.COLOR_PACKED,C.DATA_TYPE_INT8,4);t.COLOR_PACKED=t.RGBA_PACKED;t.RGB_PACKED=new t(x.COLOR_PACKED,C.DATA_TYPE_INT8,3);t.NORMAL_FLOATS=new t(x.NORMAL_FLOATS,C.DATA_TYPE_FLOAT,3);t.FILLER_1B=new t(x.FILLER,C.DATA_TYPE_UINT8,1);t.INTENSITY=new t(x.INTENSITY,C.DATA_TYPE_UINT16,1);t.CLASSIFICATION=new t(x.CLASSIFICATION,
C.DATA_TYPE_UINT8,1);t.NORMAL_SPHEREMAPPED=new t(x.NORMAL_SPHEREMAPPED,C.DATA_TYPE_UINT8,2);t.NORMAL_OCT16=new t(x.NORMAL_OCT16,C.DATA_TYPE_UINT8,2);t.NORMAL=new t(x.NORMAL,C.DATA_TYPE_FLOAT,3);t.RETURN_NUMBER=new t(x.RETURN_NUMBER,C.DATA_TYPE_UINT8,1);t.NUMBER_OF_RETURNS=new t(x.NUMBER_OF_RETURNS,C.DATA_TYPE_UINT8,1);t.SOURCE_ID=new t(x.SOURCE_ID,C.DATA_TYPE_UINT8,1);t.INDICES=new t(x.INDICES,C.DATA_TYPE_UINT32,1);t.SPACING=new t(x.SPACING,C.DATA_TYPE_FLOAT,1);F.prototype.add=function(a){this.attributes.push(a);
this.byteSize+=a.byteSize;this.size++};F.prototype.hasColors=function(){for(var a in this.attributes)if(this.attributes[a].name===x.COLOR_PACKED)return!0;return!1};F.prototype.hasNormals=function(){for(var a in this.attributes){var b=this.attributes[a];if(b===t.NORMAL_SPHEREMAPPED||b===t.NORMAL_FLOATS||b===t.NORMAL||b===t.NORMAL_OCT16)return!0}return!1};class fa{constructor(a,b,c){this.name=a;this.box=b;this.tileSize=c;this.level=this.name.length-1;this.data=new Float32Array(c*c);this.data.fill(-Infinity);
...
f;if(0===f)return null;f=0;isFinite(b)&&(f+=b*e);isFinite(a)&&(f+=a*p);isFinite(m)&&(f+=m*n);isFinite(c)&&(f+=c*d);return f}height(a){for(var b=null,c=0;c<this.mipMap.length&&(b=this.heightAtMipMapLevel(a,c),null===b);c++);return b}traverse(a,b=0){a(this,b);for(var c of this.children.filter(a=>void 0!==a))c.traverse(a,b+1)}}class ra{constructor(a){this.pointcloud=a;this.boundingBox=this.matrix=null;this.tileSize=64;this.root=null;this.version=0}expandAndFindByBox(a,b){if(0===b)return[this.root];for(var c=
...
this.root=new fa('r',this.boundingBox,this.tileSize),this.version++);var b=null,c;for(c of a)if(void 0===c.demVersion||c.demVersion<this.version){b=c;break}if(null!==b){var e=b.getBoundingBox().clone().applyMatrix4(this.matrix),d=e.getSize(new THREE.Vector3),f=this.expandAndFindByBox(e,b.getLevel());b.demVersion=this.version;a=b.geometryNode.geometry.attributes.position.array;b={boundingBox:{min:b.getBoundingBox().min.toArray(),max:b.getBoundingBox().max.toArray()},position:(new Float32Array(a)).buffer};
var g=this;u.workerPool.runTask(y.DEM,function(a){a=new Float32Array(a.data.dem.data);for(var b of f){for(var c=b.box.getSize(new THREE.Vector3),k=0;k<g.tileSize;k++)for(var p=0;p<g.tileSize;p++){var n=g.tileSize*(b.box.min.x+k/(g.tileSize-1)*c.x-e.min.x)/d.x,q=g.tileSize*(b.box.min.y+p/(g.tileSize-1)*c.y-e.min.y)/d.y;0>n||n>g.tileSize||0>q||q>g.tileSize||(n=Math.min(Math.floor(n),g.tileSize-1),q=Math.min(Math.floor(q),g.tileSize-1),b.data[k+g.tileSize*p]=a[n+g.tileSize*q])}b.createMipMap();b.mipMapNeedsUpdate=
!0}},b,[b.position])}}}class V{constructor(){this.needsTransformUpdate=!0}getChildren(){}getBoundingBox(){}isLoaded(){}isGeometryNode(){}isTreeNode(){}getLevel(){}getBoundingSphere(){}}class S extends THREE.Object3D{constructor(){super();this.dem=new ra(this)}initialized(){return null!==this.root}}class sa{constructor(){this.spacing=0;this.nodes=this.root=this.boundingBox=null;this.pointAttributes={};this.hierarchyStepSize=-1;this.serverURL=this.boundingSphere=this.projection=this.offset=this.baseDepth=
this.schema=this.loader=null;this.normalize={color:!1,intensity:!1}}}z.IDCount=0;z.prototype=Object.create(V.prototype);z.prototype.isGeometryNode=function(){return!0};z.prototype.isTreeNode=function(){return!1};z.prototype.isLoaded=function(){return this.loaded};z.prototype.getBoundingSphere=function(){return this.boundingSphere};z.prototype.getBoundingBox=function(){return this.boundingBox};z.prototype.getLevel=function(){return this.level};z.prototype.getChildren=function(){for(var a=[],b=0;8>
...
(f+='&scale='+this.scale);this.greyhoundOffset&&(e=this.greyhoundOffset,f+='&offset=['+e.x+','+e.y+','+e.z+']');var g=this,k=new XMLHttpRequest;k.overrideMimeType('text/plain');k.open('GET',f,!0);k.onload=function(a){try{var b=JSON.parse(k.responseText)||{};a=[];g.numPoints=b.n;c(b,g.name,a);b={};b[g.name]=g;for(var d=g.pcoGeometry,e=0;e<a.length;e++){var f=a[e].name,h=a[e].numPoints,w=parseInt(f.charAt(f.length-1)),r=f.substring(0,f.length-1),v=b[r],t=f.length-1,x=N.createChildAABB(v.boundingBox,
w),y=new z(f,d,x,g.scale,g.offset);y.level=t;y.numPoints=h;y.hasChildren=0<a[e].children;y.spacing=d.spacing/Math.pow(2,t);v.addChild(y);b[f]=y}g.loadPoints()}catch(Ja){u.numNodesLoading--,console.error('Potree: Exception thrown parsing points.',Ja)}};k.onerror=function(a){console.log('Potree: Failed to load file! HTTP status '+k.status+', file:'+f,a)};k.send(null)}};z.prototype.getNumPoints=function(){return this.numPoints};z.prototype.dispose=function(){if(this.geometry&&null!=this.parent){this.geometry.dispose();
this.geometry=null;this.loaded=!1;for(var a=0;a<this.oneTimeDisposeHandlers.length;a++)(0,this.oneTimeDisposeHandlers[a])();this.oneTimeDisposeHandlers=[]}};Object.assign(z.prototype,THREE.EventDispatcher.prototype);E.prototype.newerThan=function(a){a=new E(a);return this.versionMajor>a.versionMajor||this.versionMajor===a.versionMajor&&this.versionMinor>a.versionMinor?!0:!1};E.prototype.equalOrHigher=function(a){a=new E(a);return this.versionMajor>a.versionMajor||this.versionMajor===a.versionMajor&&
this.versionMinor>=a.versionMinor?!0:!1};E.prototype.upTo=function(a){return!this.newerThan(a)};class ta{constructor(a,b,c){this.version='string'===typeof a?new E(a):a;this.boundingBox=b;this.scale=c}load(a){if(!a.loaded){var b=this,c=a.getURL(),e=new XMLHttpRequest;e.overrideMimeType('text/plain');e.open('GET',c,!0);e.responseType='arraybuffer';e.overrideMimeType('text/plain; charset=x-user-defined');e.onload=function(){try{b.parse(a,e.response)}catch(d){console.error('Potree: Exception thrown parsing points.',
d),u.numNodesLoading--}};e.onerror=function(a){u.numNodesLoading--;console.error('Potree: Failed to load file.',e,c)};e.send(null)}}parse(a,b){var c=(new DataView(b,b.byteLength-4,4)).getUint32(0,!0),e=a.pcoGeometry.pointAttributes;a.numPoints=c;c=a.boundingBox;var d=new THREE.Vector3;d=a.pcoGeometry.boundingBox.getCenter(d).sub(a.boundingBox.min);b={buffer:b,pointAttributes:e,version:this.version.version,schema:a.pcoGeometry.schema,min:[c.min.x,c.min.y,c.min.z],max:[c.max.x,c.max.y,c.max.z],offset:d.toArray(),
scale:this.scale,normalize:a.pcoGeometry.normalize};u.workerPool.runTask(y.GREYHOUND,function(b){b=b.data;var c=b.attributeBuffers,d=new THREE.Box3((new THREE.Vector3).fromArray(b.tightBoundingBox.min),(new THREE.Vector3).fromArray(b.tightBoundingBox.max)),e=new THREE.BufferGeometry,f;for(f in c){var m=c[f].buffer;parseInt(f)===x.POSITION_CARTESIAN?e.addAttribute('position',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(f)===x.COLOR_PACKED?e.addAttribute('color',new THREE.BufferAttribute(new Uint8Array(m),
4,!0)):parseInt(f)===x.INTENSITY?e.addAttribute('intensity',new THREE.BufferAttribute(new Float32Array(m),1)):parseInt(f)===x.CLASSIFICATION?e.addAttribute('classification',new THREE.BufferAttribute(new Uint8Array(m),1)):parseInt(f)===x.NORMAL_SPHEREMAPPED?e.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(f)===x.NORMAL_OCT16?e.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(f)===x.NORMAL?e.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),
3)):parseInt(f)===x.INDICES?(m=new THREE.BufferAttribute(new Uint8Array(m),4),m.normalized=!0,e.addAttribute('indices',m)):parseInt(f)===x.SPACING&&(m=new THREE.BufferAttribute(new Float32Array(m),1),e.addAttribute('spacing',m))}d.max.sub(d.min);d.min.set(0,0,0);a.numPoints=b.numPoints;a.geometry=e;a.mean=new THREE.Vector3(...b.mean);a.tightBoundingBox=d;a.loaded=!0;a.loading=!1;u.numNodesLoading--},b,[b.buffer])}}class H{static getQueryParam(a){a=a.replace(/[[\]]/g,'\\$&');return(a=(new RegExp('[?&]'+
a+'(=([^&#]*)|&|#|$)')).exec(window.location.href))?a[2]?decodeURIComponent(a[2].replace(/\+/g,' ')):'':null}static createSchema(a){var b=[{name:'X',size:4,type:'signed'},{name:'Y',size:4,type:'signed'},{name:'Z',size:4,type:'signed'}];a.forEach(function(a){'COLOR_PACKED'===a?(b.push({name:'Red',size:2,type:'unsigned'}),b.push({name:'Green',size:2,type:'unsigned'}),b.push({name:'Blue',size:2,type:'unsigned'})):'INTENSITY'===a?b.push({name:'Intensity',size:2,type:'unsigned'}):'CLASSIFICATION'===a&&
b.push({name:'Classification',size:1,type:'unsigned'})});return b}static fetch(a,b){var c=new XMLHttpRequest;c.overrideMimeType('text/plain');c.open('GET',a,!0);c.onreadystatechange=function(){4===c.readyState&&(200===c.status||0===c.status?b(null,c.responseText):b(c.responseText))};c.send(null)}static fetchBinary(a,b){var c=new XMLHttpRequest;c.overrideMimeType('text/plain');c.open('GET',a,!0);c.responseType='arraybuffer';c.onreadystatechange=function(){4===c.readyState&&(200===c.status||0===c.status?
b(null,c.response):b(c.responseText))};c.send(null)}static pointSizeFrom(a){return a.reduce((a,c)=>a+c.size,0)}static getNormalization(a,b,c){var e=[{name:'X',size:4,type:'floating'},{name:'Y',size:4,type:'floating'},{name:'Z',size:4,type:'floating'},{name:'Red',size:2,type:'unsigned'},{name:'Green',size:2,type:'unsigned'},{name:'Blue',size:2,type:'unsigned'},{name:'Intensity',size:2,type:'unsigned'}];a=a+'read?depth='+b+'&schema='+JSON.stringify(e);H.fetchBinary(a,function(a,b){if(a)throw Error(a);
a=new DataView(b);b=b.byteLength-4;for(var d=H.pointSizeFrom(e),f=!1,h=!1,l=0;l<b;l+=d){if(255<a.getUint16(l+12,!0)||255<a.getUint16(l+14,!0)||255<a.getUint16(l+16,!0))f=!0;255<a.getUint16(l+18,!0)&&(h=!0);if(f&&h)break}c(null,{color:f,intensity:h})})}}N.loadInfoJSON=function(a,b){};N.load=function(a,b){try{var c=a.split('greyhound://')[1];1===c.split('http://').length&&1===c.split('https://').length&&(c='http://'+c);H.fetch(c+'info',function(a,d){if(a)throw Error(a);a=JSON.parse(d);var e=new E('1.4'),
g=a.bounds;d=a.scale||.01;Array.isArray(d)&&(d=Math.min(d[0],d[1],d[2]));H.getQueryParam('scale')&&(d=parseFloat(H.getQueryParam('scale')));var k=Math.max(8,a.baseDepth),h=['POSITION_CARTESIAN'],l=!1,m=!1,p=!1;a.schema.forEach(function(a){'Intensity'===a.name&&h.push('INTENSITY');'Classification'===a.name&&h.push('CLASSIFICATION');'Red'===a.name?l=!0:'Green'===a.name?m=!0:'Blue'===a.name&&(p=!0)});l&&m&&p&&h.push('COLOR_PACKED');var n=new sa;n.serverURL=c;n.spacing=(g[3]-g[0])/Math.pow(2,k);n.baseDepth=
k;n.hierarchyStepSize=5;n.schema=H.createSchema(h);k=H.pointSizeFrom(n.schema);n.pointAttributes=new F(h);n.pointAttributes.byteSize=k;g=new THREE.Box3((new THREE.Vector3).fromArray(g,0),(new THREE.Vector3).fromArray(g,3));k=g.min.clone();g.max.sub(g.min);g.min.set(0,0,0);n.projection=a.srs;n.boundingBox=g;n.boundingSphere=g.getBoundingSphere(new THREE.Sphere);n.scale=d;n.offset=k;n.loader=new ta(e,g,n.scale);e={};d=new z('r',n,g,d,k);d.level=0;d.hasChildren=!0;d.numPoints=a.numPoints;d.spacing=n.spacing;
n.root=d;n.root.load();e.r=d;n.nodes=e;H.getNormalization(c,a.baseDepth,function(a,c){c.color&&(n.normalize.color=!0);c.intensity&&(n.normalize.intensity=!0);b(n)})})}catch(e){console.log('Potree: Loading failed.',a,e),b()}};N.loadPointAttributes=function(a){a=a.pointAttributes;for(var b=new F,c=0;c<a.length;c++)b.add(t[a[c]]);return b};N.createChildAABB=function(a,b){var c=a.min;a=a.max;var e=(new THREE.Vector3).copy(a).sub(c).multiplyScalar(.5),d=new THREE.Vector3(e.x,0,0),f=new THREE.Vector3(0,
e.y,0),g=new THREE.Vector3(0,0,e.z),k=c,h=(new THREE.Vector3).add(c).add(e);1===b?(c=(new THREE.Vector3).copy(k).add(g),a=(new THREE.Vector3).copy(h).add(g)):3===b?(c=(new THREE.Vector3).copy(k).add(g).add(f),a=(new THREE.Vector3).copy(h).add(g).add(f)):0===b?(c=k,a=h):2===b?(c=(new THREE.Vector3).copy(k).add(f),a=(new THREE.Vector3).copy(h).add(f)):5===b?(c=(new THREE.Vector3).copy(k).add(g).add(d),a=(new THREE.Vector3).copy(h).add(g).add(d)):7===b?(c=(new THREE.Vector3).copy(k).add(e),a=(new THREE.Vector3).copy(h).add(e)):
...
e.onload=function(){try{c.parse(a,e.response)}catch(d){u.numNodesLoading--,console.error('Potree: Exception thrown parsing points.',d)}};e.onerror=function(a){u.numNodesLoading--;console.error('Potree: Failed to load file.',e,b)};e.send(null)}}parse(a,b){var c=a.pcoGeometry.pointAttributes,e=b.byteLength/a.pcoGeometry.pointAttributes.byteSize;this.version.upTo('1.5')&&(a.numPoints=e);b={buffer:b,pointAttributes:c,version:this.version.version,min:[a.boundingBox.min.x,a.boundingBox.min.y,a.boundingBox.min.z],
offset:[a.pcoGeometry.offset.x,a.pcoGeometry.offset.y,a.pcoGeometry.offset.z],scale:this.scale,spacing:a.spacing,hasChildren:a.hasChildren,name:a.name};u.workerPool.runTask(y.BINARY_DECODER,function(b){var d=b.data;if(d.error)u.numNodesLoading--,console.error('Potree: Binary worker error.');else{var e=d.attributeBuffers,k=new THREE.Box3((new THREE.Vector3).fromArray(d.tightBoundingBox.min),(new THREE.Vector3).fromArray(d.tightBoundingBox.max)),h=new THREE.BufferGeometry,l;for(l in e){var m=e[l].buffer;
parseInt(l)===x.POSITION_CARTESIAN?h.addAttribute('position',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(l)===x.COLOR_PACKED?h.addAttribute('color',new THREE.BufferAttribute(new Uint8Array(m),4,!0)):parseInt(l)===x.INTENSITY?h.addAttribute('intensity',new THREE.BufferAttribute(new Float32Array(m),1)):parseInt(l)===x.CLASSIFICATION?h.addAttribute('classification',new THREE.BufferAttribute(new Uint8Array(m),1)):parseInt(l)===x.NORMAL_SPHEREMAPPED?h.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),
3)):parseInt(l)===x.NORMAL_OCT16?h.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(l)===x.NORMAL?h.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(l)===x.INDICES?(m=new THREE.BufferAttribute(new Uint8Array(m),4),m.normalized=!0,h.addAttribute('indices',m)):parseInt(l)===x.SPACING&&(m=new THREE.BufferAttribute(new Float32Array(m),1),h.addAttribute('spacing',m))}k.max.sub(k.min);k.min.set(0,0,0);a.numPoints=b.data.buffer.byteLength/c.byteSize;
a.geometry=h;a.mean=new THREE.Vector3(...d.mean);a.tightBoundingBox=k;a.loaded=!0;a.loading=!1;a.estimatedSpacing=d.estimatedSpacing;u.numNodesLoading--}},b,[b.buffer])}}var ka=[function(a){return{position:[a.getInt32(0,!0),a.getInt32(4,!0),a.getInt32(8,!0)],intensity:a.getUint16(12,!0),classification:a.getUint8(16,!0)}},function(a){return{position:[a.getInt32(0,!0),a.getInt32(4,!0),a.getInt32(8,!0)],intensity:a.getUint16(12,!0),classification:a.getUint8(16,!0)}},function(a){return{position:[a.getInt32(0,
!0),a.getInt32(4,!0),a.getInt32(8,!0)],intensity:a.getUint16(12,!0),classification:a.getUint8(16,!0),color:[a.getUint16(20,!0),a.getUint16(22,!0),a.getUint16(24,!0)]}},function(a){return{position:[a.getInt32(0,!0),a.getInt32(4,!0),a.getInt32(8,!0)],intensity:a.getUint16(12,!0),classification:a.getUint8(16,!0),color:[a.getUint16(28,!0),a.getUint16(30,!0),a.getUint16(32,!0)]}}];R.prototype.open=function(){this.readOffset=0;return new Promise(function(a,b){setTimeout(a,0)})};R.prototype.getHeader=function(){var a=
this;return new Promise(function(b,c){setTimeout(function(){var c=a.arraybuffer,d={};d.pointsOffset=I(c,Uint32Array,96);d.pointsFormatId=I(c,Uint8Array,104);d.pointsStructSize=I(c,Uint16Array,105);d.pointsCount=I(c,Uint32Array,107);var f=131;d.scale=I(c,Float64Array,f,3);f+=24;d.offset=I(c,Float64Array,f,3);c=I(c,Float64Array,f+24,6);d.maxs=[c[0],c[2],c[4]];d.mins=[c[1],c[3],c[5]];a.header=d;b(a.header)},0)})};R.prototype.readData=function(a,b,c){var e=this;return new Promise(function(b,f){setTimeout(function(){if(!e.header)return f(Error('Cannot start reading data till a header request is issued'));
if(1>=c){a=Math.min(a,e.header.pointsCount-e.readOffset);var d=e.header.pointsOffset+e.readOffset*e.header.pointsStructSize;b({buffer:e.arraybuffer.slice(d,d+a*e.header.pointsStructSize),count:a,hasMoreData:e.readOffset+a<e.header.pointsCount});e.readOffset+=a}else{for(var k=Math.min(a*c,e.header.pointsCount-e.readOffset),h=0,l=new Uint8Array(Math.ceil(k/c)*e.header.pointsStructSize),m=0;m<k;m++)0===m%c&&(d=e.header.pointsOffset+e.readOffset*e.header.pointsStructSize,d=new Uint8Array(e.arraybuffer,
d,e.header.pointsStructSize),l.set(d,h*e.header.pointsStructSize),h++),e.readOffset++;b({buffer:l.buffer,count:h,hasMoreData:e.readOffset<e.header.pointsCount})}},0)})};R.prototype.close=function(){var a=this;return new Promise(function(b,c){a.arraybuffer=null;setTimeout(b,0)})};U.prototype.open=function(){var a=this;return new Promise(function(b,c){a.dorr({type:'open',arraybuffer:a.arraybuffer},function(a){if(1!==a.status)return c(Error('Failed to open file'));b(!0)})})};U.prototype.getHeader=function(){var a=
this;return new Promise(function(b,c){a.dorr({type:'header'},function(a){if(1!==a.status)return c(Error('Failed to get header'));b(a.header)})})};U.prototype.readData=function(a,b,c){var e=this;return new Promise(function(d,f){e.dorr({type:'read',count:a,offset:b,skip:c},function(a){if(1!==a.status)return f(Error('Failed to read data'));d({buffer:a.buffer,count:a.count,hasMoreData:a.hasMoreData})})})};U.prototype.close=function(){var a=this;return new Promise(function(b,c){a.dorr({type:'close'},function(a){if(1!==
a.status)return c(Error('Failed to close file'));b(!0)})})};J.prototype.determineFormat=function(){var a=I(this.arraybuffer,Uint8Array,104),b=(a&128)>>7,c=(a&64)>>6;if(1===b&&1===c)throw Error('Old style compression not supported');this.formatId=a&63;this.isCompressed=1===b||1===c};J.prototype.determineVersion=function(){var a=new Int8Array(this.arraybuffer,24,2);this.version=10*a[0]+a[1];this.versionAsString=a[0]+'.'+a[1]};J.prototype.open=function(){return this.loader.open()};J.prototype.getHeader=
function(){return this.loader.getHeader()};J.prototype.readData=function(a,b,c){return this.loader.readData(a,b,c)};J.prototype.close=function(){return this.loader.close()};da.prototype.getPoint=function(a){if(0>a||a>=this.pointsCount)throw Error('Point index out of range');return this.decoder(new DataView(this.arrayb,a*this.pointSize,this.pointSize))};class va{constructor(a){this.version='string'===typeof a?new E(a):a}load(a){if(!a.loaded){var b=a.pcoGeometry.pointAttributes,c=a.getURL();this.version.equalOrHigher('1.4')&&
(c+='.'+b.toLowerCase());var e=this,d=new XMLHttpRequest;d.open('GET',c,!0);d.responseType='arraybuffer';d.overrideMimeType('text/plain; charset=x-user-defined');d.onload=function(){if(d.response instanceof ArrayBuffer)try{e.parse(a,d.response)}catch(f){console.error('Potree: Exception thrown parsing points.',f),u.numNodesLoading--}else u.numNodesLoading--,console.log('Potree: LASLAZLoader xhr response is not a ArrayBuffer.')};d.onerror=function(){u.numNodesLoading--;console.log('Potree: LASLAZLoader failed to load file, '+
d.status+', file: '+c)};d.send(null)}}parse(a,b){var c=new J(b),e=new La(a);c.open().then(a=>{c.isOpen=!0;return c}).catch(a=>{console.log('Potree: Failed to open file.')}).then(a=>a.getHeader().then(function(b){return[a,b]})).then(a=>{let b=a[0],c=a[1],d=0;var h=function(){return b.readData(1E6,0,1).then(function(a){e.push(new da(a.buffer,c.pointsFormatId,c.pointsStructSize,a.count,c.scale,c.offset,c.mins,c.maxs));d+=a.count;if(a.hasMoreData)return h();c.totalRead=d;c.versionAsString=b.versionAsString;
c.isCompressed=b.isCompressed;return[b,c,e]})};return h()}).then(a=>{let b=a[0];return b.close().then(function(){b.isOpen=!1;return a.slice(1)}).catch(a=>{if(b.isOpen)return b.close().then(function(){b.isOpen=!1;throw a;});throw a;})})}handle(a,b){}}class La{constructor(a){this.node=a}push(a){var b=this,c={buffer:a.arrayb,numPoints:a.pointsCount,pointSize:a.pointSize,pointFormatID:2,scale:a.scale,offset:a.offset,mins:a.mins,maxs:a.maxs},e=u.workerPool.getWorker(y.LAS_DECODER);e.onmessage=function(c){var d=
new THREE.BufferGeometry,g=a.pointsCount,k=new Float32Array(c.data.position),h=new Uint8Array(c.data.color),l=new Float32Array(c.data.intensity),m=new Uint8Array(c.data.classification),p=new Uint8Array(c.data.returnNumber),n=new Uint8Array(c.data.numberOfReturns),q=new Uint16Array(c.data.pointSourceID),w=new Uint8Array(c.data.indices);d.addAttribute('position',new THREE.BufferAttribute(k,3));d.addAttribute('color',new THREE.BufferAttribute(h,4,!0));d.addAttribute('intensity',new THREE.BufferAttribute(l,
1));d.addAttribute('classification',new THREE.BufferAttribute(m,1));d.addAttribute('returnNumber',new THREE.BufferAttribute(p,1));d.addAttribute('numberOfReturns',new THREE.BufferAttribute(n,1));d.addAttribute('pointSourceID',new THREE.BufferAttribute(q,1));d.addAttribute('indices',new THREE.BufferAttribute(w,4));d.attributes.indices.normalized=!0;k=new THREE.Box3((new THREE.Vector3).fromArray(c.data.tightBoundingBox.min),(new THREE.Vector3).fromArray(c.data.tightBoundingBox.max));d.boundingBox=b.node.boundingBox;
b.node.tightBoundingBox=k;b.node.geometry=d;b.node.numPoints=g;b.node.loaded=!0;b.node.loading=!1;u.numNodesLoading--;b.node.mean=new THREE.Vector3(...c.data.mean);u.workerPool.returnWorker(y.LAS_DECODER,e)};e.postMessage(c,[c.buffer])}}class wa{constructor(){this.octreeDir=this.url=null;this.spacing=0;this.pointAttributes=this.nodes=this.root=this.boundingBox=null;this.hierarchyStepSize=-1;this.loader=null}}class O extends V{constructor(a,b,c){super();this.id=O.IDCount++;this.name=a;this.index=parseInt(a.charAt(a.length-
1));this.pcoGeometry=b;this.geometry=null;this.boundingBox=c;this.boundingSphere=c.getBoundingSphere(new THREE.Sphere);this.children={};this.numPoints=0;this.level=null;this.loaded=!1;this.oneTimeDisposeHandlers=[]}isGeometryNode(){return!0}getLevel(){return this.level}isTreeNode(){return!1}isLoaded(){return this.loaded}getBoundingSphere(){return this.boundingSphere}getBoundingBox(){return this.boundingBox}getChildren(){for(var a=[],b=0;8>b;b++)this.children[b]&&a.push(this.children[b]);return a}getURL(){var a=
'',b=this.pcoGeometry.loader.version;b.equalOrHigher('1.5')?a=this.pcoGeometry.octreeDir+'/'+this.getHierarchyPath()+'/'+this.name:b.equalOrHigher('1.4')?a=this.pcoGeometry.octreeDir+'/'+this.name:b.upTo('1.3')&&(a=this.pcoGeometry.octreeDir+'/'+this.name);return a}getHierarchyPath(){for(var a='r/',b=this.pcoGeometry.hierarchyStepSize,c=this.name.substr(1),e=Math.floor(c.length/b),d=0;d<e;d++)a+=c.substr(d*b,b)+'/';return a=a.slice(0,-1)}addChild(a){this.children[a.index]=a;a.parent=this}load(){if(!(!0===
this.loading||!0===this.loaded||u.numNodesLoading>=u.maxNodesLoading)){this.loading=!0;u.numNodesLoading++;try{this.pcoGeometry.loader.version.equalOrHigher('1.5')?0===this.level%this.pcoGeometry.hierarchyStepSize&&this.hasChildren?this.loadHierachyThenPoints():this.loadPoints():this.loadPoints()}catch(a){u.numNodesLoading--,console.error('Potree: Exception thrown loading points file.',a)}}}loadPoints(){this.pcoGeometry.loader.load(this)}loadHierachyThenPoints(){var a=this;if(0===a.level%a.pcoGeometry.hierarchyStepSize){var b=
a.pcoGeometry.octreeDir+'/'+a.getHierarchyPath()+'/'+a.name+'.hrc',c=new XMLHttpRequest;c.open('GET',b,!0);c.responseType='arraybuffer';c.overrideMimeType('text/plain; charset=x-user-defined');c.onload=function(b){try{var d=c.response,e=new DataView(d);b=[];var g=e.getUint8(0),k=e.getUint32(1,!0);a.numPoints=k;b.push({children:g,numPoints:k,name:a.name});g=[];for(k=5;0<b.length;){for(var h=b.shift(),l=1,m=0;8>m;m++){if(0!==(h.children&l)){var p=h.name+m,n=e.getUint8(k),q=e.getUint32(k+1,!0);b.push({children:n,
numPoints:q,name:p});g.push({children:n,numPoints:q,name:p});k+=5}l*=2}if(k===d.byteLength)break}d={};d[a.name]=a;var w=a.pcoGeometry;for(m=0;m<g.length;m++){var r=g[m].name,v=g[m].numPoints,t=parseInt(r.charAt(r.length-1)),x=r.substring(0,r.length-1),y=d[x],z=r.length-1,A=Z.createChildAABB(y.boundingBox,t),B=new O(r,w,A);B.level=z;B.numPoints=v;B.hasChildren=0<g[m].children;B.spacing=w.spacing/Math.pow(2,z);y.addChild(B);d[r]=B}a.loadPoints()}catch(Ia){u.numNodesLoading--,console.error('Potree: Exception thrown parsing points.',
Ia)}};c.onerror=function(a){u.numNodesLoading--;console.error('Potree: Failed to load file.',c.status,b,a)};c.send(null)}}getNumPoints(){return this.numPoints}dispose(){if(this.geometry&&null!=this.parent){this.geometry.dispose();this.geometry=null;this.loaded=!1;for(var a=0;a<this.oneTimeDisposeHandlers.length;a++)(0,this.oneTimeDisposeHandlers[a])();this.oneTimeDisposeHandlers=[]}}}O.IDCount=0;Object.assign(O.prototype,THREE.EventDispatcher.prototype);class Z{static load(a,b){var c=new wa;c.url=
a;var e=new XMLHttpRequest;e.overrideMimeType('text/plain');e.open('GET',a,!0);e.onload=function(){var d=JSON.parse(e.responseText),f=new E(d.version);0===d.octreeDir.indexOf('http')?c.octreeDir=d.octreeDir:c.octreeDir=a+'/../'+d.octreeDir;c.spacing=d.spacing;c.hierarchyStepSize=d.hierarchyStepSize;c.pointAttributes=d.pointAttributes;var g=new THREE.Vector3(d.boundingBox.lx,d.boundingBox.ly,d.boundingBox.lz),k=new THREE.Vector3(d.boundingBox.ux,d.boundingBox.uy,d.boundingBox.uz),h=new THREE.Box3(g,
k);k=h.clone();d.tightBoundingBox&&(k.min.copy(new THREE.Vector3(d.tightBoundingBox.lx,d.tightBoundingBox.ly,d.tightBoundingBox.lz)),k.max.copy(new THREE.Vector3(d.tightBoundingBox.ux,d.tightBoundingBox.uy,d.tightBoundingBox.uz)));g=g.clone();h.min.sub(g);h.max.sub(g);k.min.sub(g);k.max.sub(g);c.projection=d.projection;c.boundingBox=h;c.tightBoundingBox=k;c.boundingSphere=h.getBoundingSphere(new THREE.Sphere);c.tightBoundingSphere=k.getBoundingSphere(new THREE.Sphere);c.offset=g;'LAS'===d.pointAttributes||
'LAZ'===d.pointAttributes?c.loader=new va(d.version):(c.loader=new ua(d.version,h,d.scale),c.pointAttributes=new F(c.pointAttributes));g={};k='r';h=new O(k,c,h);h.level=0;h.hasChildren=!0;h.spacing=c.spacing;h.numPoints=f.upTo('1.5')?d.hierarchy[0][1]:0;c.root=h;c.root.load();g[k]=h;if(f.upTo('1.4'))for(f=1;f<d.hierarchy.length;f++){k=d.hierarchy[f][0];var l=d.hierarchy[f][1];h=parseInt(k.charAt(k.length-1));var m=k.substring(0,k.length-1);m=g[m];var p=k.length-1;h=Z.createChildAABB(m.boundingBox,
h);h=new O(k,c,h);h.level=p;h.numPoints=l;h.spacing=c.spacing/Math.pow(2,p);m.addChild(h);g[k]=h}c.nodes=g;b(c)};e.onerror=function(c){Global.numNodesLoading--;console.log('Potree: loading file failed.',a,c);b()};e.send(null)}static loadPointAttributes(a){a=a.pointAttributes;for(var b=new F,c=0;c<a.length;c++)b.add(t[a[c]]);return b}static createChildAABB(a,b){var c=a.min.clone();a=a.max.clone();var e=(new THREE.Vector3).subVectors(a,c);0<(b&1)?c.z+=e.z/2:a.z-=e.z/2;0<(b&2)?c.y+=e.y/2:a.y-=e.y/2;
0<(b&4)?c.x+=e.x/2:a.x-=e.x/2;return new THREE.Box3(c,a)}}class xa{load(a){if(!a.loaded){var b=a.url()+'.bin',c=new XMLHttpRequest;c.open('GET',b,!0);c.responseType='arraybuffer';c.overrideMimeType('text/plain; charset=x-user-defined');c.onreadystatechange=()=>{4===c.readyState&&(200===c.status?this.parse(a,c.response):console.log('Failed '+b+': '+c.status))};try{c.send(null)}catch(e){console.log('Failed request: '+e)}}}parse(a,b){var c=u.workerPool.getWorker(y.EPT_BINARY_DECODER);c.onmessage=function(b){var d=
new THREE.BufferGeometry,e=b.data.numPoints,g=new Float32Array(b.data.position);d.addAttribute('position',new THREE.BufferAttribute(g,3));g=new Uint8Array(b.data.indices);d.addAttribute('indices',new THREE.BufferAttribute(g,4));b.data.color&&(g=new Uint8Array(b.data.color),d.addAttribute('color',new THREE.BufferAttribute(g,4,!0)));b.data.intensity&&(g=new Float32Array(b.data.intensity),d.addAttribute('intensity',new THREE.BufferAttribute(g,1)));b.data.classification&&(g=new Uint8Array(b.data.classification),
d.addAttribute('classification',new THREE.BufferAttribute(g,1)));b.data.returnNumber&&(g=new Uint8Array(b.data.returnNumber),d.addAttribute('returnNumber',new THREE.BufferAttribute(g,1)));b.data.numberOfReturns&&(g=new Uint8Array(b.data.numberOfReturns),d.addAttribute('numberOfReturns',new THREE.BufferAttribute(g,1)));b.data.pointSourceId&&(g=new Uint16Array(b.data.pointSourceId),d.addAttribute('pointSourceID',new THREE.BufferAttribute(g,1)));d.attributes.indices.normalized=!0;g=new THREE.Box3((new THREE.Vector3).fromArray(b.data.tightBoundingBox.min),
(new THREE.Vector3).fromArray(b.data.tightBoundingBox.max));a.doneLoading(d,g,e,new THREE.Vector3(...b.data.mean));u.workerPool.returnWorker(y.EPT_BINARY_DECODER,c)};b={buffer:b,schema:a.ept.schema,scale:a.ept.eptScale,offset:a.ept.eptOffset,mins:(a=>[a.x,a.y,a.z])(a.key.b.min)};c.postMessage(b,[b.buffer])}}class ya{load(a){if(!a.loaded){var b=a.url()+'.laz',c=new XMLHttpRequest;c.open('GET',b,!0);c.responseType='arraybuffer';c.overrideMimeType('text/plain; charset=x-user-defined');c.onreadystatechange=
()=>{4===c.readyState&&(200===c.status?this.parse(a,c.response):console.log('Failed '+b+': '+c.status))};c.send(null)}}parse(a,b){var c=new J(b),e=new Ma(a);c.open().then(()=>{c.isOpen=!0;return c.getHeader()}).then(b=>{var d=0,g=a=>[a.x,a.y,a.z],k=g(a.key.b.min),h=g(a.key.b.max),l=()=>c.readData(1E6,0,1).then(function(a){var f=new da(a.buffer,b.pointsFormatId,b.pointsStructSize,a.count,b.scale,b.offset,k,h);f.extraBytes=b.extraBytes;f.pointsFormatId=b.pointsFormatId;e.push(f);d+=a.count;if(a.hasMoreData)return l();
b.totalRead=d;b.versionAsString=c.versionAsString;b.isCompressed=c.isCompressed;return null});return l()}).then(()=>c.close()).then(()=>c.isOpen=!1).catch(a=>{console.log('Error reading LAZ:',a);if(c.isOpen)c.close().then(()=>{c.isOpen=!1;throw a;});else throw a;})}}class Ma{constructor(a){this.node=a}push(a){var b=u.workerPool.getWorker(y.EPT_LAS_ZIP_DECODER);b.onmessage=c=>{var d=new THREE.BufferGeometry,e=a.pointsCount,g=new Float32Array(c.data.position),k=new Uint8Array(c.data.color),h=new Float32Array(c.data.intensity),
l=new Uint8Array(c.data.classification),m=new Uint8Array(c.data.returnNumber),p=new Uint8Array(c.data.numberOfReturns),n=new Uint16Array(c.data.pointSourceID),q=new Uint8Array(c.data.indices);d.addAttribute('position',new THREE.BufferAttribute(g,3));d.addAttribute('color',new THREE.BufferAttribute(k,4,!0));d.addAttribute('intensity',new THREE.BufferAttribute(h,1));d.addAttribute('classification',new THREE.BufferAttribute(l,1));d.addAttribute('returnNumber',new THREE.BufferAttribute(m,1));d.addAttribute('numberOfReturns',
