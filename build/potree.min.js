...
r.dispatchEvent({type:'transformation_changed',target:r}))}else z.set(r,{number:0,transform:r.matrixWorld.clone()});for(;0<m.size();){r=m.pop();q=r.node;var u=r.parent;let w=a[r.pointcloud];var x=q.getBoundingBox();let C=n[r.pointcloud];var t=k[r.pointcloud].intersectsBox(x);x=w.maxLevel||Infinity;var v=q.getLevel();t=(t=(t=t&&!(e+q.getNumPoints()>Potree.pointBudget))&&!(d.get(w)+q.getNumPoints()>w.pointBudget))&&v<x;q.spacing?l=Math.min(l,q.spacing):q.geometryNode&&q.geometryNode.spacing&&(l=Math.min(l,
q.geometryNode.spacing));if(e+q.getNumPoints()>Potree.pointBudget)break;if(t)for(e+=q.getNumPoints(),x=d.get(w),d.set(w,x+q.getNumPoints()),w.numVisibleNodes++,w.numVisiblePoints+=q.getNumPoints(),!q.isGeometryNode()||u&&!u.isTreeNode()||(q.isLoaded()&&2>p?(q=w.toTreeNode(q,u),p++):(h.push(q),g.push(q))),q.isTreeNode()&&(Potree.getLRU().touch(q.geometryNode),q.sceneNode.visible=!0,q.sceneNode.material=w.material,f.push(q),w.visibleNodes.push(q),void 0===q._transformVersion&&(q._transformVersion=-1),
u=z.get(w),q._transformVersion!==u.number&&(q.sceneNode.updateMatrix(),q.sceneNode.matrixWorld.multiplyMatrices(w.matrixWorld,q.sceneNode.matrix),q._transformVersion=u.number),w.showBoundingBox&&!q.boundingBoxNode&&q.getBoundingBox?(u=new Potree.Box3Helper(q.getBoundingBox()),u.matrixAutoUpdate=!1,w.boundingBoxNodes.push(u),q.boundingBoxNode=u,q.boundingBoxNode.matrix.copy(w.matrixWorld)):w.showBoundingBox?(q.boundingBoxNode.visible=!0,q.boundingBoxNode.matrix.copy(w.matrixWorld)):!w.showBoundingBox&&
q.boundingBoxNode&&(q.boundingBoxNode.visible=!1)),u=q.getChildren(),x=0;x<u.length;x++){v=u[x];var y=0;if(b.isPerspectiveCamera){t=v.getBoundingSphere(new THREE.Sphere);var A=t.center,B=C.x-A.x;y=C.y-A.y;A=C.z-A.z;B=Math.sqrt(B*B+y*y+A*A);t=t.radius;y=.5*c/(Math.tan(b.fov*Math.PI/180/2)*B)*t;if(y<w.minimumNodePixelSize)continue;0>B-t&&(y=Number.MAX_VALUE)}else t=v.getBoundingBox(),B=v.getBoundingSphere(new THREE.Sphere).center.distanceTo(C),y=t.max.clone().sub(t.min).length()/B;m.push({pointcloud:r.pointcloud,
node:v,parent:q,weight:y})}}a=a.filter((a)=>a.generateDEM&&a.dem instanceof Potree.DEM);for(var C of a)a=C.visibleNodes.filter((a)=>4>=a.getLevel()),C.dem.update(a);for(C=0;C<Math.min(Potree.maxNodesLoading,h.length);C++)h[C].load();return{visibleNodes:f,numVisiblePoints:e,lowestSpacing:l}};
...
k+2+12*m);k=a.slice(k+2,k+2+12*m);for(var x=0;x<m;x++){var t=f.fromValue(k.readUInt16LE(12*x)),v=d.fromValue(k.readUInt16LE(12*x+2)),y=k.readUInt32LE(12*x+4),A=k.readUInt32LE(12*x+8),B=v.bytes*y;if(4>=B)B=A;else{var C=new Uint8Array(B);C.set(a.slice(A,A+B));B=new (g.get(v))(C.buffer);v===d.ASCII&&(B=String.fromCharCode(...B))}t=new h(t,v,y,A,B);c.push(t)}console.log('nextIFDOffset',n);if(0===n)break;k=n;b++}k=(a)=>{for(let b of c)if(b.tag===a)return b;return null};b=k(f.IMAGE_WIDTH,c).value;m=k(f.IMAGE_HEIGHT,
c).value;k(f.COMPRESSION,c);n=k(f.ROWS_PER_STRIP,c).value;t=k(f.STRIP_OFFSETS,c);v=k(f.STRIP_BYTE_COUNTS,c);k=Math.ceil(m/n);x=[];for(y=0;y<v.count;y++){A=v.type;B=v.offset+y*A.bytes;var w;A===d.SHORT?w=a.readUInt16LE(B):A===d.LONG&&(w=a.readUInt32LE(B));x.push(w)}w=[];for(v=0;v<t.count;v++){y=t.type;A=t.offset+v*y.bytes;var D;y===d.SHORT?D=a.readUInt16LE(A):y===d.LONG&&(D=a.readUInt32LE(A));w.push(D)}D=new Uint8Array(b*m*3);t=0;for(v=0;v<k;v++)for(y=w[v],y=a.slice(y,y+x[v]),A=3*b,B=0;B<n;B++)if(C=
...
if(n instanceof Potree.PointCloudOctreeNode)var z=0===Object.keys(n.children).length;else n instanceof Potree.PointCloudArena4DNode&&(z=n.geometryNode.isLeaf);f.setUniform('uIsLeafNode',z);var r=f.uniformLocations.modelMatrix;r&&(m.set(p.elements),g.uniformMatrix4fv(r,!1,m));r=f.uniformLocations.modelViewMatrix;m.set(l.elements);g.uniformMatrix4fv(r,!1,m);if(b.clipPolygons&&0<b.clipPolygons.length){r=[];k=[];for(var q of b.clipPolygons){d=q.viewMatrix;var u=q.projMatrix.clone().multiply(d).multiply(p);
r.push(q.markers.length);k.push(u)}u=[].concat(...k.map((a)=>a.elements));var x=Array(24*b.clipPolygons.length);for(k=0;k<b.clipPolygons.length;k++){q=b.clipPolygons[k];for(var t=0;t<q.markers.length;t++)x[24*k+3*t]=q.markers[t].position.x,x[24*k+(3*t+1)]=q.markers[t].position.y,x[24*k+(3*t+2)]=q.markers[t].position.z}g.uniform1iv(f.uniformLocations['uClipPolygonVCount[0]'],r);g.uniformMatrix4fv(f.uniformLocations['uClipPolygonWVP[0]'],!1,u);g.uniform3fv(f.uniformLocations['uClipPolygonVertices[0]'],
x)}f.setUniform1f('uLevel',c);f.setUniform1f('uNodeSpacing',n.geometryNode.estimatedSpacing);f.setUniform1f('uPCIndex',k);if(0<h.length){k=f.uniformLocations['uShadowMap[0]'];f.setUniform3f('uShadowColor',b.uniforms.uShadowColor.value);c=Array(h.length).fill(5).map((a,b)=>a+b);g.uniform1iv(k,c);for(k=0;k<h.length;k++)r=c[k],u=a.properties.get(h[k].target.texture).__webglTexture,g.activeTexture(g[`TEXTURE${r}`]),g.bindTexture(g.TEXTURE_2D,u);c=h.map((a)=>a.camera.matrixWorldInverse).map((a)=>(new THREE.Matrix4).multiplyMatrices(a,
p));u=[].concat(...c.map((a)=>a.elements));g.uniformMatrix4fv(f.uniformLocations['uShadowWorldView[0]'],!1,u);u=[].concat(...h.map((a)=>a.camera.projectionMatrix.elements));g.uniformMatrix4fv(f.uniformLocations['uShadowProj[0]'],!1,u)}c=n.geometryNode.geometry;r=null;if(this.buffers.has(c)){r=this.buffers.get(c);for(var v in c.attributes)c.attributes[v].version>r.vbos.get(v).version&&this.updateBuffer(g,c)}else r=this.createBuffer(g,c),this.buffers.set(c,r);g.bindVertexArray(r.vao);g.drawArrays(g.POINTS,
