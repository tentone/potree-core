...
b&&(this.defines.set(a,b),this.updateShaderSource()):this.removeDefine(a)}removeDefine(a){this.defines.delete(a)}updateShaderSource(){this.vertexShader=this.getDefines()+Potree.Shaders['pointcloud.vs'];this.fragmentShader=this.getDefines()+Potree.Shaders['pointcloud.fs'];this.depthFunc=THREE.LessEqualDepth;this.needsUpdate=this.depthWrite=this.depthTest=!0}getDefines(){let a=[];this.logarithmicDepthBuffer&&(a.push('#define USE_LOGDEPTHBUF'),a.push('#define USE_LOGDEPTHBUF_EXT'));this.pointSizeType===
Potree.PointSizeType.FIXED?a.push('#define fixed_point_size'):this.pointSizeType===Potree.PointSizeType.ATTENUATED?a.push('#define attenuated_point_size'):this.pointSizeType===Potree.PointSizeType.ADAPTIVE&&a.push('#define adaptive_point_size');this.shape===Potree.PointShape.SQUARE?a.push('#define square_point_shape'):this.shape===Potree.PointShape.CIRCLE?a.push('#define circle_point_shape'):this.shape===Potree.PointShape.PARABOLOID&&a.push('#define paraboloid_point_shape');this._useEDL&&a.push('#define use_edl');
this._snapEnabled&&a.push('#define snap_enabled');this._pointColorType===Potree.PointColorType.RGB?a.push('#define color_type_rgb'):this._pointColorType===Potree.PointColorType.COLOR?a.push('#define color_type_color'):this._pointColorType===Potree.PointColorType.DEPTH?a.push('#define color_type_depth'):this._pointColorType===Potree.PointColorType.HEIGHT?a.push('#define color_type_height'):this._pointColorType===Potree.PointColorType.INTENSITY?a.push('#define color_type_intensity'):this._pointColorType===
Potree.PointColorType.INTENSITY_GRADIENT?a.push('#define color_type_intensity_gradient'):this._pointColorType===Potree.PointColorType.LOD?a.push('#define color_type_lod'):this._pointColorType===Potree.PointColorType.POINT_INDEX?a.push('#define color_type_point_index'):this._pointColorType===Potree.PointColorType.CLASSIFICATION?a.push('#define color_type_classification'):this._pointColorType===Potree.PointColorType.RETURN_NUMBER?a.push('#define color_type_return_number'):this._pointColorType===Potree.PointColorType.SOURCE?
a.push('#define color_type_source'):this._pointColorType===Potree.PointColorType.NORMAL?a.push('#define color_type_normal'):this._pointColorType===Potree.PointColorType.PHONG?a.push('#define color_type_phong'):this._pointColorType===Potree.PointColorType.RGB_HEIGHT?a.push('#define color_type_rgb_height'):this._pointColorType===Potree.PointColorType.COMPOSITE&&a.push('#define color_type_composite');this._treeType===Potree.TreeType.OCTREE?a.push('#define tree_type_octree'):this._treeType===Potree.TreeType.KDTREE&&
a.push('#define tree_type_kdtree');this.weighted&&a.push('#define weighted_splats');for(let [,b]of this.defines)a.push(b);return a.join('\n')}setClipBoxes(a){if(a){var b=this.clipBoxes.length!==a.length&&(0===a.length||0===this.clipBoxes.length);this.uniforms.clipBoxCount.value=this.clipBoxes.length;this.clipBoxes=a;b&&this.updateShaderSource();this.uniforms.clipBoxes.value=new Float32Array(16*this.clipBoxes.length);for(b=0;b<this.clipBoxes.length;b++)this.uniforms.clipBoxes.value.set(a[b].inverse.elements,
16*b);for(a=0;a<this.uniforms.clipBoxes.value.length;a++)Number.isNaN(this.uniforms.clipBoxes.value[a])&&(this.uniforms.clipBoxes.value[a]=Infinity)}}setClipPolygons(a,b){a&&(this.clipPolygons=a,this.clipPolygons.length!==a.length&&this.updateShaderSource())}get gradient(){return this._gradient}set gradient(a){this._gradient!==a&&(this._gradient=a,this.gradientTexture=Potree.PointCloudMaterial.generateGradientTexture(this._gradient),this.uniforms.gradient.value=this.gradientTexture)}get useOrthographicCamera(){return this.uniforms.useOrthographicCamera.value}set useOrthographicCamera(a){this.uniforms.useOrthographicCamera.value!==
...
a)}get far(){return this.uniforms.far.value}set far(a){this.uniforms.far.value!==a&&(this.uniforms.far.value=a)}get opacity(){return this.uniforms.uOpacity.value}set opacity(a){this.uniforms&&this.uniforms.uOpacity&&this.uniforms.uOpacity.value!==a&&(this.uniforms.uOpacity.value=a,this.updateShaderSource(),this.dispatchEvent({type:'opacity_changed',target:this}),this.dispatchEvent({type:'material_property_changed',target:this}))}get pointColorType(){return this._pointColorType}set pointColorType(a){this._pointColorType!==
a&&(this._pointColorType=a,this.updateShaderSource(),this.dispatchEvent({type:'point_color_type_changed',target:this}),this.dispatchEvent({type:'material_property_changed',target:this}))}get pointSizeType(){return this._pointSizeType}set pointSizeType(a){this._pointSizeType!==a&&(this._pointSizeType=a,this.updateShaderSource(),this.dispatchEvent({type:'point_size_type_changed',target:this}),this.dispatchEvent({type:'material_property_changed',target:this}))}get useEDL(){return this._useEDL}set useEDL(a){this._useEDL!==
a&&(this._useEDL=a,this.updateShaderSource())}get color(){return this.uniforms.uColor.value}set color(a){this.uniforms.uColor.value.equals(a)||(this.uniforms.uColor.value.copy(a),this.dispatchEvent({type:'color_changed',target:this}),this.dispatchEvent({type:'material_property_changed',target:this}))}get shape(){return this._shape}set shape(a){this._shape!==a&&(this._shape=a,this.updateShaderSource(),this.dispatchEvent({type:'point_shape_changed',target:this}),this.dispatchEvent({type:'material_property_changed',
target:this}))}get treeType(){return this._treeType}set treeType(a){this._treeType!==a&&(this._treeType=a,this.updateShaderSource())}get bbSize(){return this.uniforms.bbSize.value}set bbSize(a){this.uniforms.bbSize.value=a}get size(){return this.uniforms.size.value}set size(a){this.uniforms.size.value!==a&&(this.uniforms.size.value=a,this.dispatchEvent({type:'point_size_changed',target:this}),this.dispatchEvent({type:'material_property_changed',target:this}))}get elevationRange(){return this.uniforms.elevationRange.value}set elevationRange(a){if(this.uniforms.elevationRange.value[0]!==
...
if(!u)break;k=a.properties.get(u).__webglTexture;l=a.properties.get(l).__webglTexture;q=h[n];d.activeTexture(d[`TEXTURE${g[n]}`]);d.bindTexture(d.TEXTURE_2D,k);d.activeTexture(d[`TEXTURE${q}`]);d.bindTexture(d.TEXTURE_2D,l)}n=[].concat(...f.uniforms.uSnapView.value.map((a)=>a.elements));d.uniformMatrix4fv(p.uniformLocations['uSnapView[0]'],!1,n);n=[].concat(...f.uniforms.uSnapProj.value.map((a)=>a.elements));d.uniformMatrix4fv(p.uniformLocations['uSnapProj[0]'],!1,n);n=[].concat(...f.uniforms.uSnapProjInv.value.map((a)=>
a.elements));d.uniformMatrix4fv(p.uniformLocations['uSnapProjInv[0]'],!1,n);n=[].concat(...f.uniforms.uSnapViewInv.value.map((a)=>a.elements));d.uniformMatrix4fv(p.uniformLocations['uSnapViewInv[0]'],!1,n)}this.renderNodes(a,b,c,m,e,p);d.activeTexture(d.TEXTURE2);d.bindTexture(d.TEXTURE_2D,null);d.activeTexture(d.TEXTURE0)}};
