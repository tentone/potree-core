...
var f=d.header.pointsOffset+d.readOffset*d.header.pointsStructSize;b({buffer:d.arraybuffer.slice(f,f+a*d.header.pointsStructSize),count:a,hasMoreData:d.readOffset+a<d.header.pointsCount});d.readOffset+=a}else{for(var g=Math.min(a*c,d.header.pointsCount-d.readOffset),h=0,l=new Uint8Array(Math.ceil(g/c)*d.header.pointsStructSize),k=0;k<g;k++)0===k%c&&(f=d.header.pointsOffset+d.readOffset*d.header.pointsStructSize,f=new Uint8Array(d.arraybuffer,f,d.header.pointsStructSize),l.set(f,h*d.header.pointsStructSize),
h++),d.readOffset++;b({buffer:l.buffer,count:h,hasMoreData:d.readOffset<d.header.pointsCount})}},0)})};d.prototype.close=function(){var a=this;return new Promise(function(b,c){a.arraybuffer=null;setTimeout(b,0)})};var f=function(a){this.arraybuffer=a;this.ww=Potree.workerPool.getWorker(Potree.scriptPath+'/workers/LASLAZWorker.js');this.nextCB=null;var b=this;this.ww.onmessage=function(a){null!==b.nextCB&&(b.nextCB(a.data),b.nextCB=null)};this.dorr=function(a,c){b.nextCB=c;b.ww.postMessage(a)}};f.prototype.open=
...
Potree.updateVisibility=function(a,b,c){let e=0,d=new Map(a.map((a)=>[a,0])),f=[],g=[],h=[],l=Infinity;var m=Potree.updateVisibilityStructures(a,b,c);let k=m.frustums,n=m.camObjPositions;m=m.priorityQueue;let p=0;c=c.domElement.clientHeight;Potree._pointcloudTransformVersion||(Potree._pointcloudTransformVersion=new Map);let t=Potree._pointcloudTransformVersion;for(var r of a)if(r.visible)if(r.updateMatrixWorld(),t.has(r)){var q=t.get(r);q.transform.equals(r.matrixWorld)||(q.number++,q.transform.copy(r.matrixWorld),
r.dispatchEvent({type:'transformation_changed',target:r}))}else t.set(r,{number:0,transform:r.matrixWorld.clone()});for(;0<m.size();){r=m.pop();q=r.node;var B=r.parent;let x=a[r.pointcloud];var v=q.getBoundingBox();let C=n[r.pointcloud];var u=k[r.pointcloud].intersectsBox(v);v=x.maxLevel||Infinity;var w=q.getLevel();u=(u=(u=u&&!(e+q.getNumPoints()>Potree.pointBudget))&&!(d.get(x)+q.getNumPoints()>x.pointBudget))&&w<v;q.spacing?l=Math.min(l,q.spacing):q.geometryNode&&q.geometryNode.spacing&&(l=Math.min(l,
q.geometryNode.spacing));if(e+q.getNumPoints()>Potree.pointBudget)break;if(u)for(e+=q.getNumPoints(),v=d.get(x),d.set(x,v+q.getNumPoints()),x.numVisibleNodes++,x.numVisiblePoints+=q.getNumPoints(),!q.isGeometryNode()||B&&!B.isTreeNode()||(q.isLoaded()&&2>p?(q=x.toTreeNode(q,B),p++):(h.push(q),g.push(q))),q.isTreeNode()&&(Potree.getLRU().touch(q.geometryNode),q.sceneNode.visible=!0,q.sceneNode.material=x.material,f.push(q),x.visibleNodes.push(q),void 0===q._transformVersion&&(q._transformVersion=-1),
B=t.get(x),q._transformVersion!==B.number&&(q.sceneNode.updateMatrix(),q.sceneNode.matrixWorld.multiplyMatrices(x.matrixWorld,q.sceneNode.matrix),q._transformVersion=B.number),x.showBoundingBox&&!q.boundingBoxNode&&q.getBoundingBox?(B=new Potree.Box3Helper(q.getBoundingBox()),B.matrixAutoUpdate=!1,x.boundingBoxNodes.push(B),q.boundingBoxNode=B,q.boundingBoxNode.matrix.copy(x.matrixWorld)):x.showBoundingBox?(q.boundingBoxNode.visible=!0,q.boundingBoxNode.matrix.copy(x.matrixWorld)):!x.showBoundingBox&&
q.boundingBoxNode&&(q.boundingBoxNode.visible=!1)),B=q.getChildren(),v=0;v<B.length;v++){w=B[v];var y=0;if(b.isPerspectiveCamera){u=w.getBoundingSphere(new THREE.Sphere);var z=u.center,A=C.x-z.x;y=C.y-z.y;z=C.z-z.z;A=Math.sqrt(A*A+y*y+z*z);u=u.radius;y=.5*c/(Math.tan(b.fov*Math.PI/180/2)*A)*u;if(y<x.minimumNodePixelSize)continue;0>A-u&&(y=Number.MAX_VALUE)}else u=w.getBoundingBox(),A=w.getBoundingSphere(new THREE.Sphere).center.distanceTo(C),y=u.max.clone().sub(u.min).length()/A;m.push({pointcloud:r.pointcloud,
node:w,parent:q,weight:y})}}a=a.filter((a)=>a.generateDEM&&a.dem instanceof Potree.DEM);for(var C of a)a=C.visibleNodes.filter((a)=>4>=a.getLevel()),C.dem.update(a);for(C=0;C<Math.min(Potree.maxNodesLoading,h.length);C++)h[C].load();return{visibleNodes:f,numVisiblePoints:e,lowestSpacing:l}};
...
a.uniforms[b].value}};function LRUItem(a){this.next=this.previous=null;this.node=a}function LRU(){this.last=this.first=null;this.items={};this.numPoints=this.elements=0}LRU.prototype.size=function(){return this.elements};LRU.prototype.contains=function(a){return null==this.items[a.id]};
...
m.applyMatrix4(p.sceneNode.matrixWorld);m.applyMatrix4(b.matrixWorldInverse);l=new THREE.Ray(b.position,b.getWorldDirection(Potree.tempVector3));l=f(l,m);m=m.center.distanceTo(b.position)+m.radius;null===l&&(l=m);l=Math.max(l,m);g.has(p.getLevel())?(m=g.get(p.getLevel()),m=Math.max(m,l),g.set(p.getLevel(),m)):g.set(p.getLevel(),l);p.geometryNode.hasChildren||h.set(p,{distance:l,i:d})}for(let [d,e]of h)if(a=d.getLevel(),b=e.distance,f=e.i,!(4>a))for(let [a,d]of g)b<1.2*d&&(c[4*f+3]=a);Potree.measureTimings&&
...
3));b=new THREE.LineBasicMaterial;this.frustum=new THREE.LineSegments(a,b);this.add(this.frustum);this.update()}update(){this.light.updateMatrix();this.light.updateMatrixWorld();var a=this.light.position,b=(new THREE.Vector3).addVectors(light.position,this.light.getWorldDirection(Potree.tempVector3).multiplyScalar(-1));b=(new THREE.Quaternion).setFromRotationMatrix((new THREE.Matrix4).lookAt(a,b,new THREE.Vector3(0,0,1)));this.setRotationFromQuaternion(b);this.position.copy(a);a=0<this.light.distance?
this.light.distance:1E3;b=a*Math.tan(.5*this.light.angle);this.frustum.scale.set(b,b,a)}};
...
k+2+12*m);k=a.slice(k+2,k+2+12*m);for(var v=0;v<m;v++){var u=f.fromValue(k.readUInt16LE(12*v)),w=d.fromValue(k.readUInt16LE(12*v+2)),y=k.readUInt32LE(12*v+4),z=k.readUInt32LE(12*v+8),A=w.bytes*y;if(4>=A)A=z;else{var C=new Uint8Array(A);C.set(a.slice(z,z+A));A=new (g.get(w))(C.buffer);w===d.ASCII&&(A=String.fromCharCode(...A))}u=new h(u,w,y,z,A);c.push(u)}console.log('nextIFDOffset',n);if(0===n)break;k=n;b++}k=(a)=>{for(let b of c)if(b.tag===a)return b;return null};b=k(f.IMAGE_WIDTH,c).value;m=k(f.IMAGE_HEIGHT,
c).value;k(f.COMPRESSION,c);n=k(f.ROWS_PER_STRIP,c).value;u=k(f.STRIP_OFFSETS,c);w=k(f.STRIP_BYTE_COUNTS,c);k=Math.ceil(m/n);v=[];for(y=0;y<w.count;y++){z=w.type;A=w.offset+y*z.bytes;var x;z===d.SHORT?x=a.readUInt16LE(A):z===d.LONG&&(x=a.readUInt32LE(A));v.push(x)}x=[];for(w=0;w<u.count;w++){y=u.type;z=u.offset+w*y.bytes;var D;y===d.SHORT?D=a.readUInt16LE(z):y===d.LONG&&(D=a.readUInt32LE(z));x.push(D)}D=new Uint8Array(b*m*3);u=0;for(w=0;w<k;w++)for(y=x[w],y=a.slice(y,y+v[w]),z=3*b,A=0;A<n;A++)if(C=
y.slice(A*z,A*z+z),D.set(C,u*z),C.length===z)u++;else break;console.log(`width: ${b}`);console.log(`height: ${m}`);console.log(`numStrips: ${k}`);console.log('stripByteCounts',v.join(', '));console.log('stripOffsets',x.join(', '));a=new l;a.width=b;a.height=m;a.buffer=D;a.metadata=c;return a}}class k{constructor(){}static toTiffBuffer(a,b={}){let c=new Uint8Array([73,73,42,0,8,0,0,0]),[e,g]=[a.width,a.height];var k=[new h(f.IMAGE_WIDTH,d.SHORT,1,null,e),new h(f.IMAGE_HEIGHT,d.SHORT,1,null,g),new h(f.BITS_PER_SAMPLE,
...
m;a.Exporter=k;return a}({});Potree.XHRFactory={config:{withCredentials:!1,customHeaders:[{header:null,value:null}]},createXMLHttpRequest:function(){let a=new XMLHttpRequest;a.overrideMimeType('text/plain');if(this.config.customHeaders&&Array.isArray(this.config.customHeaders)&&0<this.config.customHeaders.length){let b=a.open,c=this.config.customHeaders;a.open=function(){b.apply(this,[].slice.call(arguments));c.forEach(function(b){b.header&&b.value&&a.setRequestHeader(b.header,b.value)})}}return a}};Potree.BasicGroup=class extends THREE.Mesh{constructor(){super(new THREE.Geometry,new THREE.MeshBasicMaterial({opacity:1,wireframe:!0,transparent:!0}));this.rotation.set(-Math.PI/2,0,0);this.frustumCulled=!0;this.pointclouds=[];this.pointBudget=1E4;this.nodeSize=30;this.nodeLoadRate=2}setPointBudget(a){this.pointBudget=a}onBeforeRender(a,b,c,e,d,f){for(b=0;b<this.pointclouds.length;b++)e=this.pointclouds[b],e.minimumNodePixelSize=this.nodeSize,e.showBoundingBox=!1,e.generateDEM=!1;Potree.pointBudget=
this.pointBudget;Potree.pointLoadLimit=this.pointBudget*this.nodeLoadRate;Potree.updatePointClouds(this.pointclouds,c,a)}recalculateBoxGeometry(){var a=this.getBoundingBox(),b=a.getSize(new THREE.Vector3),c=a.getCenter(new THREE.Vector3);a=new THREE.Matrix4;a.makeTranslation(c.x,-c.z,c.y);b=new THREE.BoxBufferGeometry(b.x,b.z,b.y);b.applyMatrix(a);this.geometry=b}add(a){THREE.Object3D.prototype.add.call(this,a);a instanceof Potree.PointCloudTree&&(this.pointclouds.push(a),this.recalculateBoxGeometry())}remove(a){THREE.Object3D.prototype.remove.call(this,
a);a instanceof Potree.PointCloudTree&&(a=this.pointclouds.indexOf(a),-1!==a&&(this.pointclouds.splice(a,1),this.recalculateBoxGeometry()))}getBoundingBox(){var a=new THREE.Box3;this.updateMatrixWorld(!0);for(var b=0;b<this.pointclouds.length;b++){var c=this.pointclouds[b];c.updateMatrixWorld(!0);c=Potree.utils.computeTransformedBoundingBox(c.pcoGeometry.tightBoundingBox?c.pcoGeometry.tightBoundingBox:c.boundingBox,c.matrixWorld);a.union(c)}return a}estimateHeightAt(a){var b=null,c=Infinity,e;for(e of this.pointclouds)if(void 0!==
e.root.geometryNode){var d=null,f=Infinity,g=a.clone().sub(e.position);g.z=0;g=new THREE.Ray(g,new THREE.Vector3(0,0,1));for(var h=[e.root];0<h.length;){var l=h.pop(),m=l.getBoundingBox();if(g.intersectBox(m)){m=l.geometryNode.mean.z+e.position.z+l.geometryNode.boundingBox.min.z;l.geometryNode.spacing<=f&&(d=m,f=l.geometryNode.spacing);for(var k of Object.keys(l.children))l.children[k].geometryNode&&h.push(l.children[k])}}if(null===b||f<c)b=d,c=f}return b}};Potree.Group=class extends Potree.BasicGroup{constructor(){super();this.buffers=new Map;this.shaders=new Map;this.textures=new Map;this.types=new Map;this.types.set(Float32Array,5126);this.types.set(Uint8Array,5121);this.types.set(Uint16Array,5123)}getExtensions(a){a.getExtension('EXT_frag_depth');a.getExtension('WEBGL_depth_texture');var b=a.getExtension('OES_vertex_array_object');a.createVertexArray=b.createVertexArrayOES.bind(b);a.bindVertexArray=b.bindVertexArrayOES.bind(b)}onBeforeRender(a,b,
...
c instanceof Potree.PointCloudTree?a.push(c):(c=c.children.filter((a)=>a.visible),b.push(...c))}return{octrees:a}}renderNodes(a,b,c,e,d,f){var g=a.context;b=b.material;var h=[];d=d.matrixWorldInverse;var l=new THREE.Matrix4,m=new Float32Array(16),k;for(k of c)if(void 0===Potree.debug.allowedNodes||Potree.debug.allowedNodes.includes(k.name)){var n=k.sceneNode.matrixWorld;l.multiplyMatrices(d,n);e&&(c=e.offsets.get(k),f.setUniform1f('uVNStart',c));c=k.getLevel();f.setUniform('uDebug',!0===k.debug);
if(k instanceof Potree.PointCloudOctreeNode)var p=0===Object.keys(k.children).length;else k instanceof Potree.PointCloudArena4DNode&&(p=k.geometryNode.isLeaf);f.setUniform('uIsLeafNode',p);var t=f.uniformLocations.modelMatrix;t&&(m.set(n.elements),g.uniformMatrix4fv(t,!1,m));t=f.uniformLocations.modelViewMatrix;m.set(l.elements);g.uniformMatrix4fv(t,!1,m);if(b.clipPolygons&&0<b.clipPolygons.length){t=[];var r=[];for(v of b.clipPolygons){d=v.viewMatrix;var q=v.projMatrix.clone().multiply(d).multiply(n);
t.push(v.markers.length);r.push(q)}q=[].concat(...r.map((a)=>a.elements));var B=Array(24*b.clipPolygons.length);for(r=0;r<b.clipPolygons.length;r++){var v=b.clipPolygons[r];for(var u=0;u<v.markers.length;u++)B[24*r+3*u]=v.markers[u].position.x,B[24*r+(3*u+1)]=v.markers[u].position.y,B[24*r+(3*u+2)]=v.markers[u].position.z}g.uniform1iv(f.uniformLocations['uClipPolygonVCount[0]'],t);g.uniformMatrix4fv(f.uniformLocations['uClipPolygonWVP[0]'],!1,q);g.uniform3fv(f.uniformLocations['uClipPolygonVertices[0]'],
B)}f.setUniform1f('uLevel',c);f.setUniform1f('uNodeSpacing',k.geometryNode.estimatedSpacing);f.setUniform1f('uPCIndex',r);if(0<h.length){r=f.uniformLocations['uShadowMap[0]'];f.setUniform3f('uShadowColor',b.uniforms.uShadowColor.value);c=Array(h.length).fill(5).map((a,b)=>a+b);g.uniform1iv(r,c);for(r=0;r<h.length;r++)t=c[r],q=a.properties.get(h[r].target.texture).__webglTexture,g.activeTexture(g[`TEXTURE${t}`]),g.bindTexture(g.TEXTURE_2D,q);c=h.map((a)=>a.camera.matrixWorldInverse).map((a)=>(new THREE.Matrix4).multiplyMatrices(a,
n));q=[].concat(...c.map((a)=>a.elements));g.uniformMatrix4fv(f.uniformLocations['uShadowWorldView[0]'],!1,q);q=[].concat(...h.map((a)=>a.camera.projectionMatrix.elements));g.uniformMatrix4fv(f.uniformLocations['uShadowProj[0]'],!1,q)}c=k.geometryNode.geometry;t=null;if(this.buffers.has(c)){t=this.buffers.get(c);for(var w in c.attributes)c.attributes[w].version>t.vbos.get(w).version&&this.updateBuffer(g,c)}else t=this.createBuffer(g,c),this.buffers.set(c,t);g.bindVertexArray(t.vao);g.drawArrays(g.POINTS,
0,t.numElements)}g.bindVertexArray(null)}renderOctree(a,b,c,e){var d=a.context,f=b.material,g=e.matrixWorldInverse,h=e.matrixWorld,l=e.projectionMatrix,m=(new THREE.Matrix4).getInverse(l);new THREE.Matrix4;var k=null,n=null,p=0;0<=f.pointSizeType&&(f.pointSizeType===Potree.PointSizeType.ADAPTIVE||f.pointColorType===Potree.PointColorType.LOD)&&(n=b.computeVisibilityTextureData(c,e),k=f.visibleNodesTexture,k.image.data.set(n.data),k.needsUpdate=!0);if(!this.shaders.has(f)){var [t,r]=[f.vertexShader,
f.fragmentShader];k=new Potree.Shader(d,'pointcloud',t,r);this.shaders.set(f,k)}k=this.shaders.get(f);t=f.vertexShader;r=f.fragmentShader;var q=['#define num_shadowmaps0','#define num_snapshots'+(f.snapEnabled?f.numSnapshots:0),'#define num_clipboxes'+(f.clipBoxes&&f.clipBoxes.length?f.clipBoxes.length:0),'#define num_clipspheres0','#define num_clippolygons'+(f.clipPolygons&&f.clipPolygons.length?f.clipPolygons.length:0)].join('\n');k.update(`${q}\n${t}`,`${q}\n${r}`);f.needsUpdate=!1;for(var B of Object.keys(f.uniforms))if(t=
f.uniforms[B],'t'==t.type&&(t=t.value))this.textures.has(t)||(r=new Potree.WebGLTexture(d,t),this.textures.set(t,r)),this.textures.get(t).update();d.useProgram(k.program);1>f.opacity?(d.enable(d.BLEND),d.blendFunc(d.SRC_ALPHA,d.ONE),d.depthMask(!1),d.disable(d.DEPTH_TEST)):(d.disable(d.BLEND),d.depthMask(!0),d.enable(d.DEPTH_TEST));k.setUniformMatrix4('projectionMatrix',l);k.setUniformMatrix4('viewMatrix',g);k.setUniformMatrix4('uViewInv',h);k.setUniformMatrix4('uProjInv',m);g=f.screenHeight;k.setUniform1f('uScreenWidth',
...
p);d.bindTexture(g.target,g.id);p++;if(!0===f.snapEnabled){l=k.uniformLocations['uSnapshot[0]'];m=k.uniformLocations['uSnapshotDepth[0]'];g=Array(5).fill(p).map((a,b)=>a+b);h=Array(5).fill(1+Math.max(...g)).map((a,b)=>a+b);p=1+Math.max(...h);d.uniform1iv(l,g);d.uniform1iv(m,h);for(p=0;5>p;p++){t=f.uniforms.uSnapshot.value[p];m=f.uniforms.uSnapshotDepth.value[p];if(!t)break;l=a.properties.get(t).__webglTexture;m=a.properties.get(m).__webglTexture;B=h[p];d.activeTexture(d[`TEXTURE${g[p]}`]);d.bindTexture(d.TEXTURE_2D,
l);d.activeTexture(d[`TEXTURE${B}`]);d.bindTexture(d.TEXTURE_2D,m)}p=[].concat(...f.uniforms.uSnapView.value.map((a)=>a.elements));d.uniformMatrix4fv(k.uniformLocations['uSnapView[0]'],!1,p);p=[].concat(...f.uniforms.uSnapProj.value.map((a)=>a.elements));d.uniformMatrix4fv(k.uniformLocations['uSnapProj[0]'],!1,p);p=[].concat(...f.uniforms.uSnapProjInv.value.map((a)=>a.elements));d.uniformMatrix4fv(k.uniformLocations['uSnapProjInv[0]'],!1,p);p=[].concat(...f.uniforms.uSnapViewInv.value.map((a)=>a.elements));
