...
a.fragmentShader,a.vertexShader='#define USE_LOGDEPTHBUF\n#define USE_LOGDEPTHBUF_EXT\n#define EPSILON 1e-6\n'+a.vertexShader)}getDefines(){let a=[];this.logarithmicDepthBuffer&&(a.push('#define USE_LOGDEPTHBUF'),a.push('#define USE_LOGDEPTHBUF_EXT'),a.push('#define EPSILON 1e-6'));this.pointSizeType===Potree.PointSizeType.FIXED?a.push('#define fixed_point_size'):this.pointSizeType===Potree.PointSizeType.ATTENUATED?a.push('#define attenuated_point_size'):this.pointSizeType===Potree.PointSizeType.ADAPTIVE&&
a.push('#define adaptive_point_size');this.shape===Potree.PointShape.SQUARE?a.push('#define square_point_shape'):this.shape===Potree.PointShape.CIRCLE?a.push('#define circle_point_shape'):this.shape===Potree.PointShape.PARABOLOID&&a.push('#define paraboloid_point_shape');this._useEDL&&a.push('#define use_edl');this._snapEnabled&&a.push('#define snap_enabled');this._pointColorType===Potree.PointColorType.RGB?a.push('#define color_type_rgb'):this._pointColorType===Potree.PointColorType.COLOR?a.push('#define color_type_color'):
this._pointColorType===Potree.PointColorType.DEPTH?a.push('#define color_type_depth'):this._pointColorType===Potree.PointColorType.HEIGHT?a.push('#define color_type_height'):this._pointColorType===Potree.PointColorType.INTENSITY?a.push('#define color_type_intensity'):this._pointColorType===Potree.PointColorType.INTENSITY_GRADIENT?a.push('#define color_type_intensity_gradient'):this._pointColorType===Potree.PointColorType.LOD?a.push('#define color_type_lod'):this._pointColorType===Potree.PointColorType.POINT_INDEX?
a.push('#define color_type_point_index'):this._pointColorType===Potree.PointColorType.CLASSIFICATION?a.push('#define color_type_classification'):this._pointColorType===Potree.PointColorType.RETURN_NUMBER?a.push('#define color_type_return_number'):this._pointColorType===Potree.PointColorType.SOURCE?a.push('#define color_type_source'):this._pointColorType===Potree.PointColorType.NORMAL?a.push('#define color_type_normal'):this._pointColorType===Potree.PointColorType.PHONG?a.push('#define color_type_phong'):
this._pointColorType===Potree.PointColorType.RGB_HEIGHT?a.push('#define color_type_rgb_height'):this._pointColorType===Potree.PointColorType.COMPOSITE&&a.push('#define color_type_composite');this._treeType===Potree.TreeType.OCTREE?a.push('#define tree_type_octree'):this._treeType===Potree.TreeType.KDTREE&&a.push('#define tree_type_kdtree');this.weighted&&a.push('#define weighted_splats');for(let [,b]of this.defines)a.push(b);return a.join('\n')}setClipBoxes(a){if(a){var b=this.clipBoxes.length!==
a.length&&(0===a.length||0===this.clipBoxes.length);this.uniforms.clipBoxCount.value=this.clipBoxes.length;this.clipBoxes=a;b&&this.updateShaderSource();this.uniforms.clipBoxes.value=new Float32Array(16*this.clipBoxes.length);for(b=0;b<this.clipBoxes.length;b++)this.uniforms.clipBoxes.value.set(a[b].inverse.elements,16*b);for(a=0;a<this.uniforms.clipBoxes.value.length;a++)Number.isNaN(this.uniforms.clipBoxes.value[a])&&(this.uniforms.clipBoxes.value[a]=Infinity)}}setClipPolygons(a,b){a&&(this.clipPolygons=
a,this.clipPolygons.length!==a.length&&this.updateShaderSource())}get gradient(){return this._gradient}set gradient(a){this._gradient!==a&&(this._gradient=a,this.gradientTexture=Potree.PointCloudMaterial.generateGradientTexture(this._gradient),this.uniforms.gradient.value=this.gradientTexture)}get useOrthographicCamera(){return this.uniforms.useOrthographicCamera.value}set useOrthographicCamera(a){this.uniforms.useOrthographicCamera.value!==a&&(this.uniforms.useOrthographicCamera.value=a)}get classification(){return this._classification}set classification(a){let b=
{};for(let c of Object.keys(a))b[c]=a[c].clone();if(void 0===this._classification)a=!1;else{a=Object.keys(b).length===Object.keys(this._classification).length;for(let c of Object.keys(b))a=(a=a&&void 0!==this._classification[c])&&b[c].equals(this._classification[c])}a||(this._classification=b,this.recomputeClassification())}recomputeClassification(){this.classificationTexture=Potree.PointCloudMaterial.generateClassificationTexture(this._classification);this.uniforms.classificationLUT.value=this.classificationTexture;
this.dispatchEvent({type:'material_property_changed',target:this})}get numSnapshots(){return this._numSnapshots}set numSnapshots(a){this._numSnapshots=a}get snapEnabled(){return this._snapEnabled}set snapEnabled(a){this._snapEnabled!==a&&(this._snapEnabled=a,this.updateShaderSource())}get spacing(){return this.uniforms.spacing.value}set spacing(a){this.uniforms.spacing.value!==a&&(this.uniforms.spacing.value=a)}get useClipBox(){return this._useClipBox}set useClipBox(a){this._useClipBox!==a&&(this._useClipBox=
a,this.updateShaderSource())}get clipTask(){return this.uniforms.clipTask.value}set clipTask(a){this.uniforms.clipTask.value=a}get clipMethod(){return this.uniforms.clipMethod.value}set clipMethod(a){this.uniforms.clipMethod.value=a}get weighted(){return this._weighted}set weighted(a){this._weighted!==a&&(this._weighted=a,this.updateShaderSource())}get fov(){return this.uniforms.fov.value}set fov(a){this.uniforms.fov.value!==a&&(this.uniforms.fov.value=a,this.updateShaderSource())}get screenWidth(){return this.uniforms.screenWidth.value}set screenWidth(a){this.uniforms.screenWidth.value!==
