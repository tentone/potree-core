...
offset:[a.pcoGeometry.offset.x,a.pcoGeometry.offset.y,a.pcoGeometry.offset.z],scale:this.scale,spacing:a.spacing,hasChildren:a.hasChildren,name:a.name};u.workerPool.runTask(y.BINARY_DECODER,function(b){var d=b.data;if(void 0!==d.error)u.numNodesLoading--,console.error('Potree: Binary worker error.',d);else{var e=d.attributeBuffers,k=new THREE.Box3((new THREE.Vector3).fromArray(d.tightBoundingBox.min),(new THREE.Vector3).fromArray(d.tightBoundingBox.max)),h=new THREE.BufferGeometry,l;for(l in e){var m=
