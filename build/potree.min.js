...
e.onload=function(){try{c.parse(a,e.response)}catch(d){console.error('Potree: Exception thrown parsing points.',d),u.numNodesLoading--}};e.onerror=function(a){u.numNodesLoading--;console.error('Potree: Failed to load file.',e,b)};e.send(null)}}parse(a,b){var c=a.pcoGeometry.pointAttributes,e=b.byteLength/a.pcoGeometry.pointAttributes.byteSize;this.version.upTo('1.5')&&(a.numPoints=e);b={buffer:b,pointAttributes:c,version:this.version.version,min:[a.boundingBox.min.x,a.boundingBox.min.y,a.boundingBox.min.z],
offset:[a.pcoGeometry.offset.x,a.pcoGeometry.offset.y,a.pcoGeometry.offset.z],scale:this.scale,spacing:a.spacing,hasChildren:a.hasChildren,name:a.name};u.workerPool.runTask(y.BINARY_DECODER,function(b){var d=b.data,e=d.attributeBuffers,k=new THREE.Box3((new THREE.Vector3).fromArray(d.tightBoundingBox.min),(new THREE.Vector3).fromArray(d.tightBoundingBox.max)),h=new THREE.BufferGeometry,l;for(l in e){var m=e[l].buffer;parseInt(l)===x.POSITION_CARTESIAN?h.addAttribute('position',new THREE.BufferAttribute(new Float32Array(m),
3)):parseInt(l)===x.COLOR_PACKED?h.addAttribute('color',new THREE.BufferAttribute(new Uint8Array(m),4,!0)):parseInt(l)===x.INTENSITY?h.addAttribute('intensity',new THREE.BufferAttribute(new Float32Array(m),1)):parseInt(l)===x.CLASSIFICATION?h.addAttribute('classification',new THREE.BufferAttribute(new Uint8Array(m),1)):parseInt(l)===x.NORMAL_SPHEREMAPPED?h.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(l)===x.NORMAL_OCT16?h.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),
3)):parseInt(l)===x.NORMAL?h.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(l)===x.INDICES?(m=new THREE.BufferAttribute(new Uint8Array(m),4),m.normalized=!0,h.addAttribute('indices',m)):parseInt(l)===x.SPACING&&(m=new THREE.BufferAttribute(new Float32Array(m),1),h.addAttribute('spacing',m))}k.max.sub(k.min);k.min.set(0,0,0);a.numPoints=b.data.buffer.byteLength/c.byteSize;a.geometry=h;a.mean=new THREE.Vector3(...d.mean);a.tightBoundingBox=k;a.loaded=!0;a.loading=!1;
