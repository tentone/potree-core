...
Potree.PointCloudTreeNode=class{constructor(){this.needsTransformUpdate=!0}getChildren(){throw Error('override function');}getBoundingBox(){throw Error('override function');}isLoaded(){throw Error('override function');}isGeometryNode(){throw Error('override function');}isTreeNode(){throw Error('override function');}getLevel(){throw Error('override function');}getBoundingSphere(){throw Error('override function');}};
...
l;a.Exporter=m;return a}({});Potree.PointCloudMaterial=class extends THREE.RawShaderMaterial{constructor(a={}){super();this.visibleNodesTexture=Potree.utils.generateDataTexture(2048,1,new THREE.Color(16777215));this.visibleNodesTexture.minFilter=THREE.NearestFilter;this.visibleNodesTexture.magFilter=THREE.NearestFilter;let b=function(a,b){return void 0!==a?a:b},c=b(a.size,1),e=b(a.minSize,2),d=b(a.maxSize,50);a=b(a.treeType,Potree.TreeType.OCTREE);this._pointSizeType=Potree.PointSizeType.FIXED;this._shape=Potree.PointShape.SQUARE;
