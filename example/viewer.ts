...
import { PointCloudOctree, Potree } from '../source/index';

export class Viewer {
	/**
	 * The element where we will insert our canvas.
	 */
	private targetElem: HTMLElement | undefined;
	 ...
	 */
	public camera: PerspectiveCamera = new PerspectiveCamera(45, NaN, 0.1, 1000);
	...
	public resize = () => {
		...
		const { width, height } = this.targetElem.getBoundingClientRect();
	 ...
	 *
	 * @param targetEl - The element into which we should add the canvas where we will render the scene.
	 */
	public initialize(targetEl: HTMLElement): void {
		if (this.targetElem || !targetEl) {
			return;
		}

		this.targetElem = targetEl;
	...
	public destroy(): void {
		if (this.targetElem) {
			this.targetElem.removeChild(this.renderer.domElement);
	 ...
	 */
	public load(fileName: string, baseUrl: string): Promise<PointCloudOctree> {
