	...
	static moveTo(scene, endPosition, endTarget)
	{
		...
		{ // animate camera target
			...
			let target = new THREE.Vector3().addVectors(
				camera.position,
				camera.getWorldDirection(Potree.tempVector3).clone().multiplyScalar(camTargetDistance)
			);
...
Potree.PolygonClipVolume = class extends THREE.Object3D
{
	...
	addMarker()
	{
		...
		let drag = e =>
		{
			let size = e.viewer.renderer.getSize(new THREE.Vector3());
...
Potree.PointCloudSM = class PointCloudSM
{
	...
	setLight(light)
	{
		...
		let target = new THREE.Vector3().addVectors(light.position, light.getWorldDirection(Potree.tempVector3));
...
Potree.SpotLightHelper = class SpotLightHelper extends THREE.Object3D
{
	...
	update()
	{
		...
		//let target = new THREE.Vector3().addVectors(
		//	light.position,
		//	new THREE.Vector3().subVectors(light.position, this.light.getWorldDirection(Potree.tempVector3)));
		let target = new THREE.Vector3().addVectors(
			light.position, this.light.getWorldDirection(Potree.tempVector3).multiplyScalar(-1));
