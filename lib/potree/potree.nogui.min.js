...
!u.showBoundingBox&&q.boundingBoxNode&&(q.boundingBoxNode.visible=!1)),z=q.getChildren(),A=0;A<z.length;A++){x=z[A];var y=0;if(b.isPerspectiveCamera){r=x.getBoundingSphere(new THREE.Sphere);var B=r.center,C=v.x-B.x;y=v.y-B.y;B=v.z-B.z;C=Math.sqrt(C*C+y*y+B*B);r=r.radius;y=.5*c/(Math.tan(b.fov*Math.PI/180/2)*C)*r;if(y<u.minimumNodePixelSize)continue;0>C-r&&(y=Number.MAX_VALUE)}else r=x.getBoundingBox(),C=x.getBoundingSphere(new THREE.Sphere).center.distanceTo(v),y=r.max.clone().sub(r.min).length()/
...
2)}this.mipMapNeedsUpdate=!1};potree_nogui$classdecl$var4.prototype.uv=function(a){var b=this.box.getSize(new THREE.Vector3);return[(a.x-this.box.min.x)/b.x,(a.y-this.box.min.y)/b.y]};
...
this.version){c=d;break}if(null!==c){var e=c.getBoundingBox().clone().applyMatrix4(this.matrix),f=e.getSize(new THREE.Vector3),g=this.expandAndFindByBox(e,c.getLevel());c.demVersion=this.version;Potree.getDEMWorkerInstance().onmessage=function(a){a=new Float32Array(a.data.dem.data);for(var c=$jscomp.makeIterator(g),d=c.next();!d.done;d=c.next()){d=d.value;for(var h=d.box.getSize(new THREE.Vector3),n=0;n<b.tileSize;n++)for(var p=0;p<b.tileSize;p++){var w=b.tileSize*(d.box.min.x+n/(b.tileSize-1)*h.x-
e.min.x)/f.x,t=b.tileSize*(d.box.min.y+p/(b.tileSize-1)*h.y-e.min.y)/f.y;0>w||w>b.tileSize||0>t||t>b.tileSize||(w=Math.min(Math.floor(w),b.tileSize-1),t=Math.min(Math.floor(t),b.tileSize-1),d.data[n+b.tileSize*p]=a[w+b.tileSize*t])}d.createMipMap();d.mipMapNeedsUpdate=!0;Potree.getDEMWorkerInstance().working=!1}};a=c.geometryNode.geometry.attributes.position.array;c={boundingBox:{min:c.getBoundingBox().min.toArray(),max:c.getBoundingBox().max.toArray()},position:(new Float32Array(a)).buffer};a=[c.position];
Potree.getDEMWorkerInstance().working=!0;Potree.getDEMWorkerInstance().postMessage(c,a)}}};Potree.DEM=potree_nogui$classdecl$var5;Potree.PointCloudTreeNode=function(){this.needsTransformUpdate=!0};Potree.PointCloudTreeNode.prototype.getChildren=function(){throw Error('override function');};Potree.PointCloudTreeNode.prototype.getBoundingBox=function(){throw Error('override function');};Potree.PointCloudTreeNode.prototype.isLoaded=function(){throw Error('override function');};
...
h;g=new THREE.Box3((new THREE.Vector3).fromArray(g,0),(new THREE.Vector3).fromArray(g,3));h=g.min.clone();g.max.sub(g.min);g.min.set(0,0,0);p.projection=a.srs;p.boundingBox=g;p.boundingSphere=g.getBoundingSphere(new THREE.Sphere);p.scale=e;p.offset=h;console.log('Scale:',e);console.log('Offset:',h);console.log('Bounds:',g);p.loader=new Potree.GreyhoundBinaryLoader(d,g,p.scale);d={};e=new Potree.PointCloudGreyhoundGeometryNode('r',p,g,e,h);e.level=0;e.hasChildren=!0;e.numPoints=a.numPoints;e.spacing=
p.spacing;p.root=e;p.root.load();d.r=e;p.nodes=d;GreyhoundUtils.getNormalization(c,a.baseDepth,function(a,c){c.color&&(p.normalize.color=!0);c.intensity&&(p.normalize.intensity=!0);b(p)})})}catch(d){console.log("loading failed: '"+a+"'"),console.log(d),b()}};Potree.GreyhoundLoader.loadPointAttributes=function(a){a=a.pointAttributes;for(var b=new Potree.PointAttributes,c=0;c<a.length;c++)b.add(Potree.PointAttribute[a[c]]);return b};
...
var potree_nogui$classdecl$var18=function(a,b,c){Potree.PointCloudTreeNode.call(this);this.id=Potree.PointCloudOctreeGeometryNode.IDCount++;this.name=a;this.index=parseInt(a.charAt(a.length-1));this.pcoGeometry=b;this.geometry=null;this.boundingBox=c;this.boundingSphere=c.getBoundingSphere(new THREE.Sphere);this.children={};this.numPoints=0;this.level=null;this.loaded=!1;this.oneTimeDisposeHandlers=[]};$jscomp.inherits(potree_nogui$classdecl$var18,Potree.PointCloudTreeNode);
...
potree_nogui$classdecl$var21.prototype.update=function(){this.boundingBox=this.box.geometry.boundingBox;this.boundingSphere=this.boundingBox.getBoundingSphere(new THREE.Sphere);this._clip?(this.box.visible=!1,this.label.visible=!1):(this.box.visible=!0,this.label.visible=this.showVolumeLabel)};potree_nogui$classdecl$var21.prototype.raycast=function(a,b){var c=[];this.box.raycast(a,c);0<c.length&&(a=c[0],b.push({distance:a.distance,object:this,point:a.point.clone()}))};
...
potree_nogui$classdecl$var22.prototype.update=function(){this.boundingBox=this.sphere.geometry.boundingBox;this.boundingSphere=this.boundingBox.getBoundingSphere(new THREE.Sphere)};potree_nogui$classdecl$var22.prototype.raycast=function(a,b){var c=[];this.sphere.raycast(a,c);0<c.length&&(a=c[0],b.push({distance:a.distance,object:this,point:a.point.clone()}))};potree_nogui$classdecl$var22.prototype.getVolume=function(){return 4/3*Math.PI*this.scale.x*this.scale.y*this.scale.z};
Potree.SphereVolume=potree_nogui$classdecl$var22;
...
potree_nogui$classdecl$var32.prototype.nodesOnRay=function(a,b){var c=[];b=b.clone();for(var d=0;d<a.length;d++){var e=a[d],f=e.getBoundingSphere(new THREE.Sphere).clone().applyMatrix4(e.sceneNode.matrixWorld);b.intersectsSphere(f)&&c.push(e)}return c};
...
m;k.hasRight=n;k.split=t;k.isLeaf=!m&&!n;k.number=l;k.left=null;k.right=null;k.pcoGeometry=a;k.level=f.length;h=Math.max(h,k.level);0<f.length?(m=f[f.length-1],k.boundingBox=m.boundingBox.clone(),n=m.boundingBox.getSize(new THREE.Vector3),m.hasLeft&&!m.left?(m.left=k,m.children.push(k),'X'===m.split?k.boundingBox.max.x=k.boundingBox.min.x+n.x/2:'Y'===m.split?k.boundingBox.max.y=k.boundingBox.min.y+n.y/2:'Z'===m.split&&(k.boundingBox.max.z=k.boundingBox.min.z+n.z/2),m=k.boundingBox.getCenter(),n=k.boundingBox.getSize(new THREE.Vector3).length()/
2,k.boundingSphere=new THREE.Sphere(m,n)):(m.right=k,m.children.push(k),'X'===m.split?k.boundingBox.min.x+=n.x/2:'Y'===m.split?k.boundingBox.min.y+=n.y/2:'Z'===m.split&&(k.boundingBox.min.z+=n.z/2),m=k.boundingBox.getCenter(),n=k.boundingBox.getSize(new THREE.Vector3).length()/2,k.boundingSphere=new THREE.Sphere(m,n))):(g=k,g.boundingBox=a.boundingBox.clone(),m=g.boundingBox.getCenter(),n=g.boundingBox.getSize(new THREE.Vector3).length()/2,g.boundingSphere=new THREE.Sphere(m,n));m=k.boundingBox.getSize(new THREE.Vector3);
k.spacing=(m.x+m.y+m.z)/3/75;k.estimatedSpacing=k.spacing;f.push(k);if(k.isLeaf)for(k=!1;!k&&0<f.length;)f.pop(),k=f[f.length-1],k=0<f.length&&k.hasRight&&null==k.right}a.root=g;a.levels=h;a.dispatchEvent({type:'hierarchy_loaded'})}};c.send(null)};$jscomp.global.Object.defineProperties(potree_nogui$classdecl$var34.prototype,{spacing:{configurable:!0,enumerable:!0,get:function(){if(this._spacing)return this._spacing;if(this.root)return this.root.spacing},set:function(a){this._spacing=a}}});
