...
Potree.updateVisibility=function(a,b,c){var d=0,e=new Map(a.map(function(a){return[a,0]})),f=[],g=[],h=[],l=Infinity,k=Potree.updateVisibilityStructures(a,b,c),m=k.frustums,n=k.camObjPositions;k=k.priorityQueue;var p=0;c=c.domElement.clientHeight;Potree._pointcloudTransformVersion||(Potree._pointcloudTransformVersion=new Map);for(var w=Potree._pointcloudTransformVersion,t=$jscomp.makeIterator(a),q=t.next();!q.done;q=t.next())if(q=q.value,q.visible)if(q.updateMatrixWorld(),w.has(q)){var u=w.get(q);
u.transform.equals(q.matrixWorld)||(u.number++,u.transform.copy(q.matrixWorld),q.dispatchEvent({type:'transformation_changed',target:q}))}else w.set(q,{number:0,transform:q.matrixWorld.clone()});for(;0<k.size();){t=k.pop();q=t.node;var z=t.parent;u=a[t.pointcloud];var A=q.getBoundingBox(),v=n[t.pointcloud],r=m[t.pointcloud].intersectsBox(A);A=u.maxLevel||Infinity;var x=q.getLevel();r=(r=(r=r&&!(d+q.getNumPoints()>Potree.pointBudget))&&!(e.get(u)+q.getNumPoints()>u.pointBudget))&&x<A;q.spacing?l=Math.min(l,
q.spacing):q.geometryNode&&q.geometryNode.spacing&&(l=Math.min(l,q.geometryNode.spacing));if(d+q.getNumPoints()>Potree.pointBudget)break;if(r)for(d+=q.getNumPoints(),A=e.get(u),e.set(u,A+q.getNumPoints()),u.numVisibleNodes++,u.numVisiblePoints+=q.getNumPoints(),!q.isGeometryNode()||z&&!z.isTreeNode()||(q.isLoaded()&&2>p?(q=u.toTreeNode(q,z),p++):(h.push(q),g.push(q))),q.isTreeNode()&&(Potree.getLRU().touch(q.geometryNode),q.sceneNode.visible=!0,q.sceneNode.material=u.material,f.push(q),u.visibleNodes.push(q),
void 0===q._transformVersion&&(q._transformVersion=-1),z=w.get(u),q._transformVersion!==z.number&&(q.sceneNode.updateMatrix(),q.sceneNode.matrixWorld.multiplyMatrices(u.matrixWorld,q.sceneNode.matrix),q._transformVersion=z.number),u.showBoundingBox&&!q.boundingBoxNode&&q.getBoundingBox?(z=new Potree.Box3Helper(q.getBoundingBox()),z.matrixAutoUpdate=!1,u.boundingBoxNodes.push(z),q.boundingBoxNode=z,q.boundingBoxNode.matrix.copy(u.matrixWorld)):u.showBoundingBox?(q.boundingBoxNode.visible=!0,q.boundingBoxNode.matrix.copy(u.matrixWorld)):
!u.showBoundingBox&&q.boundingBoxNode&&(q.boundingBoxNode.visible=!1)),z=q.getChildren(),A=0;A<z.length;A++){x=z[A];var y=0;if(b.isPerspectiveCamera){r=x.getBoundingSphere();var B=r.center,C=v.x-B.x;y=v.y-B.y;B=v.z-B.z;C=Math.sqrt(C*C+y*y+B*B);r=r.radius;y=.5*c/(Math.tan(b.fov*Math.PI/180/2)*C)*r;if(y<u.minimumNodePixelSize)continue;0>C-r&&(y=Number.MAX_VALUE)}else r=x.getBoundingBox(),C=x.getBoundingSphere().center.distanceTo(v),y=r.max.clone().sub(r.min).length()/C;k.push({pointcloud:t.pointcloud,
node:x,parent:q,weight:y})}}a=a.filter(function(a){return a.generateDEM&&a.dem instanceof Potree.DEM});a=$jscomp.makeIterator(a);for(q=a.next();!q.done;q=a.next())b=q.value,e=b.visibleNodes.filter(function(a){return 4>=a.getLevel()}),b.dem.update(e);for(a=0;a<Math.min(Potree.maxNodesLoading,h.length);a++)h[a].load();return{visibleNodes:f,numVisiblePoints:d,lowestSpacing:l}};
...
h;c.root.load();g.r=h;if(f.upTo('1.4'))for(f=1;f<e.hierarchy.length;f++){h=e.hierarchy[f][0];l=e.hierarchy[f][1];var k=parseInt(h.charAt(h.length-1)),m=h.substring(0,h.length-1);m=g[m];var n=h.length-1;k=Potree.POCLoader.createChildAABB(m.boundingBox,k);k=new Potree.PointCloudOctreeGeometryNode(h,c,k);k.level=n;k.numPoints=l;k.spacing=c.spacing/Math.pow(2,n);m.addChild(k);g[h]=k}c.nodes=g;b(c)}};d.send(null)}catch(e){console.log("loading failed: '"+a+"'"),console.log(e),b()}};
...
var potree_nogui$classdecl$var8=function(a,b,c){this.version='string'===typeof a?new Potree.Version(a):a;this.boundingBox=b;this.scale=c};
potree_nogui$classdecl$var8.prototype.load=function(a){var b=this;if(!a.loaded){var c=a.getURL();this.version.equalOrHigher('1.4')&&(c+='.bin');var d=Potree.XHRFactory.createXMLHttpRequest();d.open('GET',c,!0);d.responseType='arraybuffer';d.overrideMimeType('text/plain; charset=x-user-defined');d.onreadystatechange=function(){if(4===d.readyState){if(200!==d.status&&0!==d.status||null===d.response)throw Error('Failed to load file! HTTP status: '+d.status+', file: '+c);b.parse(a,d.response)}};try{d.send(null)}catch(e){console.log('fehler beim laden der punktwolke: '+
e)}}};
potree_nogui$classdecl$var8.prototype.parse=function(a,b){var c=a.pcoGeometry.pointAttributes,d=b.byteLength/a.pcoGeometry.pointAttributes.byteSize;this.version.upTo('1.5')&&(a.numPoints=d);var e=Potree.scriptPath+'/workers/BinaryDecoderWorker.js',f=Potree.workerPool.getWorker(e);f.onmessage=function(b){var d=b.data,g=d.attributeBuffers,k=new THREE.Box3((new THREE.Vector3).fromArray(d.tightBoundingBox.min),(new THREE.Vector3).fromArray(d.tightBoundingBox.max));Potree.workerPool.returnWorker(e,f);
var m=new THREE.BufferGeometry,n;for(n in g){var p=g[n].buffer;parseInt(n)===Potree.PointAttributeNames.POSITION_CARTESIAN?m.addAttribute('position',new THREE.BufferAttribute(new Float32Array(p),3)):parseInt(n)===Potree.PointAttributeNames.COLOR_PACKED?m.addAttribute('color',new THREE.BufferAttribute(new Uint8Array(p),4,!0)):parseInt(n)===Potree.PointAttributeNames.INTENSITY?m.addAttribute('intensity',new THREE.BufferAttribute(new Float32Array(p),1)):parseInt(n)===Potree.PointAttributeNames.CLASSIFICATION?
m.addAttribute('classification',new THREE.BufferAttribute(new Uint8Array(p),1)):parseInt(n)===Potree.PointAttributeNames.NORMAL_SPHEREMAPPED?m.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(p),3)):parseInt(n)===Potree.PointAttributeNames.NORMAL_OCT16?m.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(p),3)):parseInt(n)===Potree.PointAttributeNames.NORMAL?m.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(p),3)):parseInt(n)===Potree.PointAttributeNames.INDICES?
(p=new THREE.BufferAttribute(new Uint8Array(p),4),p.normalized=!0,m.addAttribute('indices',p)):parseInt(n)===Potree.PointAttributeNames.SPACING&&(p=new THREE.BufferAttribute(new Float32Array(p),1),m.addAttribute('spacing',p))}k.max.sub(k.min);k.min.set(0,0,0);a.numPoints=b.data.buffer.byteLength/c.byteSize;a.geometry=m;a.mean=new (Function.prototype.bind.apply(THREE.Vector3,[null].concat($jscomp.arrayFromIterable(d.mean))));a.tightBoundingBox=k;a.loaded=!0;a.loading=!1;a.estimatedSpacing=d.estimatedSpacing;
Potree.numNodesLoading--};b={buffer:b,pointAttributes:c,version:this.version.version,min:[a.boundingBox.min.x,a.boundingBox.min.y,a.boundingBox.min.z],offset:[a.pcoGeometry.offset.x,a.pcoGeometry.offset.y,a.pcoGeometry.offset.z],scale:this.scale,spacing:a.spacing,hasChildren:a.hasChildren,name:a.name};f.postMessage(b,[b.buffer])};Potree.BinaryLoader=potree_nogui$classdecl$var8;
...
h;for(h in c){var p=c[h].buffer;parseInt(h)===Potree.PointAttributeNames.POSITION_CARTESIAN?g.addAttribute('position',new THREE.BufferAttribute(new Float32Array(p),3)):parseInt(h)===Potree.PointAttributeNames.COLOR_PACKED?g.addAttribute('color',new THREE.BufferAttribute(new Uint8Array(p),4,!0)):parseInt(h)===Potree.PointAttributeNames.INTENSITY?g.addAttribute('intensity',new THREE.BufferAttribute(new Float32Array(p),1)):parseInt(h)===Potree.PointAttributeNames.CLASSIFICATION?g.addAttribute('classification',
new THREE.BufferAttribute(new Uint8Array(p),1)):parseInt(h)===Potree.PointAttributeNames.NORMAL_SPHEREMAPPED?g.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(p),3)):parseInt(h)===Potree.PointAttributeNames.NORMAL_OCT16?g.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(p),3)):parseInt(h)===Potree.PointAttributeNames.NORMAL?g.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(p),3)):parseInt(h)===Potree.PointAttributeNames.INDICES?(p=new THREE.BufferAttribute(new Uint8Array(p),
4),p.normalized=!0,g.addAttribute('indices',p)):parseInt(h)===Potree.PointAttributeNames.SPACING&&(p=new THREE.BufferAttribute(new Float32Array(p),1),g.addAttribute('spacing',p))}d.max.sub(d.min);d.min.set(0,0,0);a.numPoints=b.numPoints;a.geometry=g;a.mean=new (Function.prototype.bind.apply(THREE.Vector3,[null].concat($jscomp.arrayFromIterable(b.mean))));a.tightBoundingBox=d;a.loaded=!0;a.loading=!1;Potree.numNodesLoading--};c=a.boundingBox;var g=a.pcoGeometry.boundingBox.getCenter().sub(a.boundingBox.min);
...
n=!1;a.schema.forEach(function(a){'Intensity'===a.name&&l.push('INTENSITY');'Classification'===a.name&&l.push('CLASSIFICATION');'Red'===a.name?k=!0:'Green'===a.name?m=!0:'Blue'===a.name&&(n=!0)});k&&m&&n&&l.push('COLOR_PACKED');var p=new Potree.PointCloudGreyhoundGeometry;p.serverURL=c;p.spacing=(g[3]-g[0])/Math.pow(2,h);p.baseDepth=h;p.hierarchyStepSize=5;p.schema=GreyhoundUtils.createSchema(l);h=GreyhoundUtils.pointSizeFrom(p.schema);p.pointAttributes=new Potree.PointAttributes(l);p.pointAttributes.byteSize=
h;g=new THREE.Box3((new THREE.Vector3).fromArray(g,0),(new THREE.Vector3).fromArray(g,3));h=g.min.clone();g.max.sub(g.min);g.min.set(0,0,0);p.projection=a.srs;p.boundingBox=g;p.boundingSphere=g.getBoundingSphere();p.scale=e;p.offset=h;console.log('Scale:',e);console.log('Offset:',h);console.log('Bounds:',g);p.loader=new Potree.GreyhoundBinaryLoader(d,g,p.scale);d={};e=new Potree.PointCloudGreyhoundGeometryNode('r',p,g,e,h);e.level=0;e.hasChildren=!0;e.numPoints=a.numPoints;e.spacing=p.spacing;p.root=
e;p.root.load();d.r=e;p.nodes=d;GreyhoundUtils.getNormalization(c,a.baseDepth,function(a,c){c.color&&(p.normalize.color=!0);c.intensity&&(p.normalize.intensity=!0);b(p)})})}catch(d){console.log("loading failed: '"+a+"'"),console.log(d),b()}};Potree.GreyhoundLoader.loadPointAttributes=function(a){a=a.pointAttributes;for(var b=new Potree.PointAttributes,c=0;c<a.length;c++)b.add(Potree.PointAttribute[a[c]]);return b};
...
5===b?(c=(new THREE.Vector3).copy(h).add(g).add(e),a=(new THREE.Vector3).copy(l).add(g).add(e)):7===b?(c=(new THREE.Vector3).copy(h).add(d),a=(new THREE.Vector3).copy(l).add(d)):4===b?(c=(new THREE.Vector3).copy(h).add(e),a=(new THREE.Vector3).copy(l).add(e)):6===b&&(c=(new THREE.Vector3).copy(h).add(e).add(f),a=(new THREE.Vector3).copy(l).add(e).add(f));return new THREE.Box3(c,a)};var potree_nogui$classdecl$var9=function(a){this.version='string'===typeof a?new Potree.Version(a):a};
potree_nogui$classdecl$var9.progressCB=function(){};
...
pointFormatID:2,scale:a.scale,offset:a.offset,mins:a.mins,maxs:a.maxs};d.postMessage(e,[e.buffer])};Potree.LasLazBatcher=potree_nogui$classdecl$var10;
...
potree_nogui$classdecl$var11.prototype.setClipBoxes=function(a){if(a){var b=this.clipBoxes.length!==a.length&&(0===a.length||0===this.clipBoxes.length);this.uniforms.clipBoxCount.value=this.clipBoxes.length;this.clipBoxes=a;b&&this.updateShaderSource();this.uniforms.clipBoxes.value=new Float32Array(16*this.clipBoxes.length);for(b=0;b<this.clipBoxes.length;b++)this.uniforms.clipBoxes.value.set(a[b].inverse.elements,16*b);for(a=0;a<this.uniforms.clipBoxes.value.length;a++)Number.isNaN(this.uniforms.clipBoxes.value[a])&&
(this.uniforms.clipBoxes.value[a]=Infinity)}};potree_nogui$classdecl$var11.prototype.setClipPolygons=function(a,b){a&&(this.clipPolygons=a,this.clipPolygons.length!==a.length&&this.updateShaderSource())};
potree_nogui$classdecl$var11.prototype.recomputeClassification=function(){this.classificationTexture=Potree.PointCloudMaterial.generateClassificationTexture(this._classification);this.uniforms.classificationLUT.value=this.classificationTexture;this.dispatchEvent({type:'material_property_changed',target:this})};
potree_nogui$classdecl$var11.generateGradientTexture=function(a){var b=document.createElement('canvas');b.width=64;b.height=64;var c=b.getContext('2d');c.rect(0,0,64,64);for(var d=c.createLinearGradient(0,0,64,64),e=0;e<a.length;e++){var f=a[e];d.addColorStop(f[0],'#'+f[1].getHexString())}c.fillStyle=d;c.fill();a=new THREE.CanvasTexture(b);a.needsUpdate=!0;a.minFilter=THREE.LinearFilter;return a};
potree_nogui$classdecl$var11.generateClassificationTexture=function(a){for(var b=new Uint8Array(262144),c=0;256>c;c++)for(var d=0;256>d;d++){var e=c+256*d;var f=a[c]?a[c]:a[c%32]?a[c%32]:a.DEFAULT;b[4*e]=255*f.x;b[4*e+1]=255*f.y;b[4*e+2]=255*f.z;b[4*e+3]=255*f.w}a=new THREE.DataTexture(b,256,256,THREE.RGBAFormat);a.magFilter=THREE.NearestFilter;a.needsUpdate=!0;return a};
potree_nogui$classdecl$var11.prototype.disableEvents=function(){void 0===this._hiddenListeners&&(this._hiddenListeners=this._listeners,this._listeners={})};potree_nogui$classdecl$var11.prototype.enableEvents=function(){this._listeners=this._hiddenListeners;this._hiddenListeners=void 0};potree_nogui$classdecl$var11.prototype.copyFrom=function(a){for(var b=$jscomp.makeIterator(this.uniforms),c=b.next();!c.done;c=b.next())c=c.value,this.uniforms[c].value=a.uniforms[c].value};
$jscomp.global.Object.defineProperties(potree_nogui$classdecl$var11.prototype,{gradient:{configurable:!0,enumerable:!0,get:function(){return this._gradient},set:function(a){this._gradient!==a&&(this._gradient=a,this.gradientTexture=Potree.PointCloudMaterial.generateGradientTexture(this._gradient),this.uniforms.gradient.value=this.gradientTexture)}},useOrthographicCamera:{configurable:!0,enumerable:!0,get:function(){return this.uniforms.useOrthographicCamera.value},set:function(a){this.uniforms.useOrthographicCamera.value!==
...
set:function(a){this.uniforms.wReturnNumber.value!==a&&(this.uniforms.wReturnNumber.value=a,this.dispatchEvent({type:'material_property_changed',target:this}))}},weightSourceID:{configurable:!0,enumerable:!0,get:function(){return this.uniforms.wSourceID.value},set:function(a){this.uniforms.wSourceID.value!==a&&(this.uniforms.wSourceID.value=a,this.dispatchEvent({type:'material_property_changed',target:this}))}}});Potree.PointCloudMaterial=potree_nogui$classdecl$var11;
var potree_nogui$classdecl$var12=function(a){a=THREE.ShaderMaterial.call(this)||this;a.setValues({uniforms:{screenWidth:{type:'f',value:0},screenHeight:{type:'f',value:0},edlStrength:{type:'f',value:1},radius:{type:'f',value:1},neighbours:{type:'2fv',value:[]},depthMap:{type:'t',value:null},uRegularColor:{type:'t',value:null},uRegularDepth:{type:'t',value:null},uEDLColor:{type:'t',value:null},uEDLDepth:{type:'t',value:null},opacity:{type:'f',value:1}},vertexShader:a.getDefines()+Potree.Shaders['edl.vs'],
fragmentShader:a.getDefines()+Potree.Shaders['edl.fs'],lights:!1});a.neighbourCount=8;return a};$jscomp.inherits(potree_nogui$classdecl$var12,THREE.ShaderMaterial);potree_nogui$classdecl$var12.prototype.getDefines=function(){return'#define NEIGHBOUR_COUNT '+this.neighbourCount+'\n'};
potree_nogui$classdecl$var12.prototype.updateShaderSource=function(){var a=this.getDefines()+Potree.Shaders['edl.vs'],b=this.getDefines()+Potree.Shaders['edl.fs'];this.setValues({vertexShader:a,fragmentShader:b});this.uniforms.neighbours.value=this.neighbours;this.needsUpdate=!0};
$jscomp.global.Object.defineProperties(potree_nogui$classdecl$var12.prototype,{neighbourCount:{configurable:!0,enumerable:!0,get:function(){return this._neighbourCount},set:function(a){if(this._neighbourCount!==a){this._neighbourCount=a;this.neighbours=new Float32Array(2*this._neighbourCount);for(a=0;a<this._neighbourCount;a++)this.neighbours[2*a]=Math.cos(2*a*Math.PI/this._neighbourCount),this.neighbours[2*a+1]=Math.sin(2*a*Math.PI/this._neighbourCount);this.updateShaderSource()}}}});
Potree.EyeDomeLightingMaterial=potree_nogui$classdecl$var12;var potree_nogui$classdecl$var13=function(a){a=THREE.ShaderMaterial.call(this)||this;a.setValues({uniforms:{near:{type:'f',value:0},far:{type:'f',value:0},screenWidth:{type:'f',value:0},screenHeight:{type:'f',value:0},map:{type:'t',value:null}},vertexShader:Potree.Shaders['blur.vs'],fragmentShader:Potree.Shaders['blur.fs']});return a};$jscomp.inherits(potree_nogui$classdecl$var13,THREE.ShaderMaterial);Potree.BlurMaterial=potree_nogui$classdecl$var13;
var potree_nogui$classdecl$var14=function(a){a=THREE.RawShaderMaterial.call(this)||this;a.setValues({uniforms:{uDepthMap:{type:'t',value:null},uWeightMap:{type:'t',value:null}},vertexShader:a.getDefines()+Potree.Shaders['normalize.vs'],fragmentShader:a.getDefines()+Potree.Shaders['normalize.fs']});return a};$jscomp.inherits(potree_nogui$classdecl$var14,THREE.RawShaderMaterial);potree_nogui$classdecl$var14.prototype.getDefines=function(){return''};
potree_nogui$classdecl$var14.prototype.updateShaderSource=function(){var a=this.getDefines()+Potree.Shaders['normalize.vs'],b=this.getDefines()+Potree.Shaders['normalize.fs'];this.setValues({vertexShader:a,fragmentShader:b});this.needsUpdate=!0};Potree.NormalizationMaterial=potree_nogui$classdecl$var14;
...
8;return a};$jscomp.inherits(potree_nogui$classdecl$var15,THREE.RawShaderMaterial);potree_nogui$classdecl$var15.prototype.getDefines=function(){return'#define NEIGHBOUR_COUNT '+this.neighbourCount+'\n'};potree_nogui$classdecl$var15.prototype.updateShaderSource=function(){var a=this.getDefines()+Potree.Shaders['normalize.vs'],b=this.getDefines()+Potree.Shaders['normalize_and_edl.fs'];this.setValues({vertexShader:a,fragmentShader:b});this.uniforms.neighbours.value=this.neighbours;this.needsUpdate=!0};
$jscomp.global.Object.defineProperties(potree_nogui$classdecl$var15.prototype,{neighbourCount:{configurable:!0,enumerable:!0,get:function(){return this._neighbourCount},set:function(a){if(this._neighbourCount!==a){this._neighbourCount=a;this.neighbours=new Float32Array(2*this._neighbourCount);for(a=0;a<this._neighbourCount;a++)this.neighbours[2*a]=Math.cos(2*a*Math.PI/this._neighbourCount),this.neighbours[2*a+1]=Math.sin(2*a*Math.PI/this._neighbourCount);this.updateShaderSource()}}}});
Potree.NormalizationEDLMaterial=potree_nogui$classdecl$var15;function LRUItem(a){this.next=this.previous=null;this.node=a}function LRU(){this.last=this.first=null;this.items={};this.numPoints=this.elements=0}LRU.prototype.size=function(){return this.elements};LRU.prototype.contains=function(a){return null==this.items[a.id]};
...
var potree_nogui$classdecl$var16=function(){Potree.PointCloudTreeNode.call(this);this.children={};this.octree=this.sceneNode=null};$jscomp.inherits(potree_nogui$classdecl$var16,Potree.PointCloudTreeNode);potree_nogui$classdecl$var16.prototype.getNumPoints=function(){return this.geometryNode.numPoints};potree_nogui$classdecl$var16.prototype.isLoaded=function(){return!0};potree_nogui$classdecl$var16.prototype.isTreeNode=function(){return!0};potree_nogui$classdecl$var16.prototype.isGeometryNode=function(){return!1};
potree_nogui$classdecl$var16.prototype.getLevel=function(){return this.geometryNode.level};potree_nogui$classdecl$var16.prototype.getBoundingSphere=function(){return this.geometryNode.boundingSphere};potree_nogui$classdecl$var16.prototype.getBoundingBox=function(){return this.geometryNode.boundingBox};potree_nogui$classdecl$var16.prototype.getChildren=function(){for(var a=[],b=0;8>b;b++)this.children[b]&&a.push(this.children[b]);return a};
potree_nogui$classdecl$var16.prototype.getPointsInBox=function(a){if(!this.sceneNode)return null;var b=this.geometryNode.buffer,c=b.offset('position'),d=b.stride,e=new DataView(b.data);a=(new THREE.Matrix4).getInverse(a.matrixWorld);a=(new THREE.Matrix4).multiplyMatrices(a,this.sceneNode.matrixWorld);for(var f=[],g=new THREE.Vector4,h=0;h<b.numElements;h++){var l=e.getFloat32(h*d+c+0,!0),k=e.getFloat32(h*d+c+4,!0),m=e.getFloat32(h*d+c+8,!0);g.set(l,k,m,1);g.applyMatrix4(a);-.5<g.x&&.5>g.x&&-.5<g.y&&
.5>g.y&&-.5<g.z&&.5>g.z&&(g.set(l,k,m,1).applyMatrix4(this.sceneNode.matrixWorld),f.push(new THREE.Vector3(g.x,g.y,g.z)))}return f};$jscomp.global.Object.defineProperties(potree_nogui$classdecl$var16.prototype,{name:{configurable:!0,enumerable:!0,get:function(){return this.geometryNode.name}}});Potree.PointCloudOctreeNode=potree_nogui$classdecl$var16;
...
Potree.PointCloudOctree.prototype.toTreeNode=function(a,b){var c=this,d=new Potree.PointCloudOctreeNode,e=new THREE.Points(a.geometry,this.material);e.name=a.name;e.position.copy(a.boundingBox.min);e.frustumCulled=!1;e.onBeforeRender=function(b,e,f,k,m,n){m.program&&(b.getContext().useProgram(m.program.program),m.program.getUniforms().map.level&&(e=a.getLevel(),m.uniforms.level.value=e,m.program.getUniforms().map.level.setValue(b.getContext(),e)),c.visibleNodeTextureOffsets&&m.program.getUniforms().map.vnStart&&
...
c);return 0>b?null:b},g=new Map,h=new Map,l=0;l<a.length;l++){var k=a[l];d.set(k,l);for(var m=[],n=0;8>n;n++){var p=k.children[n];p&&p.constructor===Potree.PointCloudOctreeNode&&a.includes(p,l)&&m.push(p)}c[4*l]=0;c[4*l+1]=0;c[4*l+2]=0;c[4*l+3]=k.getLevel();for(n=0;n<m.length;n++){p=m[n];var w=parseInt(p.geometryNode.name.substr(-1));c[4*l]+=Math.pow(2,w);0===n&&(p=a.indexOf(p,l),c[4*l+1]=p-l>>8,c[4*l+2]=(p-l)%256)}n=k.getBoundingBox().clone().getBoundingSphere();n.applyMatrix4(k.sceneNode.matrixWorld);
n.applyMatrix4(b.matrixWorldInverse);m=new THREE.Ray(b.position,b.getWorldDirection());m=f(m,n);n=n.center.distanceTo(b.position)+n.radius;null===m&&(m=n);m=Math.max(m,n);g.has(k.getLevel())?(n=g.get(k.getLevel()),n=Math.max(n,m),g.set(k.getLevel(),n)):g.set(k.getLevel(),m);k.geometryNode.hasChildren||h.set(k,{distance:m,i:l})}a=$jscomp.makeIterator(h);for(b=a.next();!b.done;b=a.next())if(f=$jscomp.makeIterator(b.value),b=f.next().value,f=f.next().value,h=b.getLevel(),b=f.distance,f=f.i,!(4>h))for(h=
...
new THREE.Scene,n=new Potree.PointCloudMaterial;n.pointColorType=Potree.PointColorType.POINT_INDEX;this.pickState={renderTarget:new THREE.WebGLRenderTarget(1,1,{minFilter:THREE.LinearFilter,magFilter:THREE.NearestFilter,format:THREE.RGBAFormat}),material:n,scene:m}}m=this.pickState;n=m.material;n.pointSizeType=k;n.shape=this.material.shape;n.size=g;n.uniforms.minSize.value=this.material.uniforms.minSize.value;n.uniforms.maxSize.value=this.material.uniforms.maxSize.value;n.classification=this.material.classification;
d.pickClipped?(n.clipBoxes=this.material.clipBoxes,n.clipTask=this.material.clipTask===Potree.ClipTask.HIGHLIGHT?Potree.ClipTask.NONE:this.material.clipTask):n.clipBoxes=[];this.updateMaterial(n,c,b,e);m.renderTarget.setSize(l,h);g=new THREE.Vector2(d.x,d.y);k=e.getContext();k.enable(k.SCISSOR_TEST);k.scissor(parseInt(g.x-(a-1)/2),parseInt(g.y-(a-1)/2),parseInt(a),parseInt(a));e.state.buffers.depth.setTest(n.depthTest);e.state.buffers.depth.setMask(n.depthWrite);e.state.setBlending(THREE.NoBlending);
e.setRenderTarget(m.renderTarget);k.clearColor(0,0,0,0);e.clearTarget(m.renderTarget,!0,!0,!0);var p=this.material;this.material=n;f.renderOctree(this,c,b,m.renderTarget);this.material=p;b=parseInt(Math.min(Math.max(0,g.x-(a-1)/2),l));f=parseInt(Math.min(Math.max(0,g.y-(a-1)/2),h));l=parseInt(Math.min(b+a,l)-b);h=parseInt(Math.min(f+a,h)-f);l=new Uint8Array(4*l*h);k.readPixels(b,f,a,a,k.RGBA,k.UNSIGNED_BYTE,l);e.setRenderTarget(null);e.resetGLState();e.setScissorTest(!1);k.disable(k.SCISSOR_TEST);
h=new Uint32Array(l.buffer);e=[];for(b=0;b<a;b++)for(f=0;f<a;f++)m=b+f*a,k=Math.pow(b-(a-1)/2,2)+Math.pow(f-(a-1)/2,2),g=l[4*m+3],l[4*m+3]=0,m=h[m],0===g&&0===m||void 0===g||void 0===m||(g={pIndex:m,pcIndex:g,distanceToCenter:k},d.all?e.push(g):0<e.length?k<e[0].distanceToCenter&&(e[0]=g):e.push(g));a=$jscomp.makeIterator(e);for(l=a.next();!l.done;l=a.next()){l=l.value;h={};if(!c[l.pcIndex])return null;f=c[l.pcIndex];b=f.sceneNode;f=f.geometryNode.geometry;for(var w in f.attributes)k=f.attributes[w],
'position'===w&&(k=new THREE.Vector3(k.array[3*l.pIndex],k.array[3*l.pIndex+1],k.array[3*l.pIndex+2]),k.applyMatrix4(b.matrixWorld),h[w]=k);l.point=h}performance.mark('pick-end');performance.measure('pick','pick-start','pick-end');return d.all?e.map(function(a){return a.point}):0===e.length?null:e[0].point};
Potree.PointCloudOctree.prototype.getFittedBoxGen=function(a){function b(b,y,r){for(;;)switch(c){case 0:A=performance.now(),z=new THREE.Box3,u=(new THREE.Matrix4).getInverse(a.matrixWorld),q=$jscomp.makeIterator(v.visibleNodes),t=q.next();case 1:if(t.done){c=3;break}w=t.value;if(w.sceneNode){c=4;break}c=2;break;case 4:p=w.geometryNode.buffer;n=p.offset('position');m=p.stride;k=new DataView(p.data);l=(new THREE.Matrix4).multiplyMatrices(u,w.sceneNode.matrixWorld);h=new THREE.Vector4;for(b=0;b<p.numElements;b++){y=
k.getFloat32(b*m+n+0,!0);r=k.getFloat32(b*m+n+4,!0);var x=k.getFloat32(b*m+n+8,!0);h.set(y,r,x,1);h.applyMatrix4(l);-.5<h.x&&.5>h.x&&-.5<h.y&&.5>h.y&&-.5<h.z&&.5>h.z&&z.expandByPoint(h)}c=5;return{value:void 0,done:!1};case 5:if(1!=b){c=6;break}c=-1;throw r;case 6:case 2:t=q.next();c=1;break;case 3:return g=z.getCenter().applyMatrix4(a.matrixWorld),f=new THREE.Object3D,f.position.copy(g),f.scale.copy(a.scale),f.rotation.copy(a.rotation),e=(new THREE.Vector3).subVectors(z.max,z.min),f.scale.multiply(e),
d=performance.now()-A,console.log('duration: ',d),c=7,{value:f,done:!1};case 7:if(1!=b){c=8;break}c=-1;throw r;case 8:c=-1;default:return{value:void 0,done:!0}}}var c=0,d,e,f,g,h,l,k,m,n,p,w,t,q,u,z,A,v=this,r={next:function(a){return b(0,a,void 0)},throw:function(a){return b(1,void 0,a)},return:function(a){throw Error('Not yet implemented');}};$jscomp.initSymbolIterator();r[Symbol.iterator]=function(){return this};return r};
Potree.PointCloudOctree.prototype.getFittedBox=function(a,b){b=Infinity;for(var c=performance.now(),d=new THREE.Box3,e=(new THREE.Matrix4).getInverse(a.matrixWorld),f=$jscomp.makeIterator(this.visibleNodes),g=f.next();!g.done;g=f.next()){var h=g.value;if(h.sceneNode&&!(h.getLevel()>b)){g=h.geometryNode.buffer;var l=g.offset('position'),k=g.stride,m=new DataView(g.data);h=(new THREE.Matrix4).multiplyMatrices(e,h.sceneNode.matrixWorld);for(var n=new THREE.Vector4,p=0;p<g.numElements;p++){var w=m.getFloat32(p*
k+l+0,!0),t=m.getFloat32(p*k+l+4,!0),q=m.getFloat32(p*k+l+8,!0);n.set(w,t,q,1);n.applyMatrix4(h);-.5<n.x&&.5>n.x&&-.5<n.y&&.5>n.y&&-.5<n.z&&.5>n.z&&d.expandByPoint(n)}}}e=d.getCenter().applyMatrix4(a.matrixWorld);b=new THREE.Object3D;b.position.copy(e);b.scale.copy(a.scale);b.rotation.copy(a.rotation);a=(new THREE.Vector3).subVectors(d.max,d.min);b.scale.multiply(a);c=performance.now()-c;console.log('duration: ',c);return b};
...
var potree_nogui$classdecl$var17=function(){this.octreeDir=this.url=null;this.spacing=0;this.pointAttributes=this.nodes=this.root=this.boundingBox=null;this.hierarchyStepSize=-1;this.loader=null};Potree.PointCloudOctreeGeometry=potree_nogui$classdecl$var17;
var potree_nogui$classdecl$var18=function(a,b,c){Potree.PointCloudTreeNode.call(this);this.id=Potree.PointCloudOctreeGeometryNode.IDCount++;this.name=a;this.index=parseInt(a.charAt(a.length-1));this.pcoGeometry=b;this.geometry=null;this.boundingBox=c;this.boundingSphere=c.getBoundingSphere();this.children={};this.numPoints=0;this.level=null;this.loaded=!1;this.oneTimeDisposeHandlers=[]};$jscomp.inherits(potree_nogui$classdecl$var18,Potree.PointCloudTreeNode);
potree_nogui$classdecl$var18.prototype.isGeometryNode=function(){return!0};potree_nogui$classdecl$var18.prototype.getLevel=function(){return this.level};potree_nogui$classdecl$var18.prototype.isTreeNode=function(){return!1};potree_nogui$classdecl$var18.prototype.isLoaded=function(){return this.loaded};potree_nogui$classdecl$var18.prototype.getBoundingSphere=function(){return this.boundingSphere};potree_nogui$classdecl$var18.prototype.getBoundingBox=function(){return this.boundingBox};
potree_nogui$classdecl$var18.prototype.getChildren=function(){for(var a=[],b=0;8>b;b++)this.children[b]&&a.push(this.children[b]);return a};potree_nogui$classdecl$var18.prototype.getURL=function(){var a='',b=this.pcoGeometry.loader.version;b.equalOrHigher('1.5')?a=this.pcoGeometry.octreeDir+'/'+this.getHierarchyPath()+'/'+this.name:b.equalOrHigher('1.4')?a=this.pcoGeometry.octreeDir+'/'+this.name:b.upTo('1.3')&&(a=this.pcoGeometry.octreeDir+'/'+this.name);return a};
potree_nogui$classdecl$var18.prototype.getHierarchyPath=function(){for(var a='r/',b=this.pcoGeometry.hierarchyStepSize,c=this.name.substr(1),d=Math.floor(c.length/b),e=0;e<d;e++)a+=c.substr(e*b,b)+'/';return a=a.slice(0,-1)};potree_nogui$classdecl$var18.prototype.addChild=function(a){this.children[a.index]=a;a.parent=this};
potree_nogui$classdecl$var18.prototype.load=function(){!0===this.loading||!0===this.loaded||Potree.numNodesLoading>=Potree.maxNodesLoading||(this.loading=!0,Potree.numNodesLoading++,this.pcoGeometry.loader.version.equalOrHigher('1.5')?0===this.level%this.pcoGeometry.hierarchyStepSize&&this.hasChildren?this.loadHierachyThenPoints():this.loadPoints():this.loadPoints())};potree_nogui$classdecl$var18.prototype.loadPoints=function(){this.pcoGeometry.loader.load(this)};
potree_nogui$classdecl$var18.prototype.loadHierachyThenPoints=function(){var a=this;if(0===a.level%a.pcoGeometry.hierarchyStepSize){var b=a.pcoGeometry.octreeDir+'/'+a.getHierarchyPath()+'/'+a.name+'.hrc',c=Potree.XHRFactory.createXMLHttpRequest();c.open('GET',b,!0);c.responseType='arraybuffer';c.overrideMimeType('text/plain; charset=x-user-defined');c.onreadystatechange=function(){if(4===c.readyState)if(200===c.status||0===c.status){var d=c.response,e=new DataView(d),f=[],g=e.getUint8(0),h=e.getUint32(1,
!0);a.numPoints=h;f.push({children:g,numPoints:h,name:a.name});g=[];for(h=5;0<f.length;){for(var l=f.shift(),k=1,m=0;8>m;m++){if(0!==(l.children&k)){var n=l.name+m,p=e.getUint8(h),w=e.getUint32(h+1,!0);f.push({children:p,numPoints:w,name:n});g.push({children:p,numPoints:w,name:n});h+=5}k*=2}if(h===d.byteLength)break}d={};d[a.name]=a;e=a.pcoGeometry;for(f=0;f<g.length;f++)h=g[f].name,l=g[f].numPoints,n=parseInt(h.charAt(h.length-1)),k=h.substring(0,h.length-1),k=d[k],m=h.length-1,n=Potree.POCLoader.createChildAABB(k.boundingBox,
n),n=new Potree.PointCloudOctreeGeometryNode(h,e,n),n.level=m,n.numPoints=l,n.hasChildren=0<g[f].children,n.spacing=e.spacing/Math.pow(2,m),k.addChild(n),d[h]=n;a.loadPoints()}else console.log('Failed to load file! HTTP status: '+c.status+', file: '+b),Potree.numNodesLoading--};try{c.send(null)}catch(d){console.log('fehler beim laden der punktwolke: '+d)}}};potree_nogui$classdecl$var18.prototype.getNumPoints=function(){return this.numPoints};
potree_nogui$classdecl$var18.prototype.dispose=function(){if(this.geometry&&null!=this.parent){this.geometry.dispose();this.geometry=null;this.loaded=!1;for(var a=0;a<this.oneTimeDisposeHandlers.length;a++)(0,this.oneTimeDisposeHandlers[a])();this.oneTimeDisposeHandlers=[]}};Potree.PointCloudOctreeGeometryNode=potree_nogui$classdecl$var18;Potree.PointCloudOctreeGeometryNode.IDCount=0;Object.assign(Potree.PointCloudOctreeGeometryNode.prototype,THREE.EventDispatcher.prototype);
...
d+'&depthEnd='+(d+this.pcoGeometry.hierarchyStepSize+2);this.scale&&(f+='&scale='+this.scale);this.greyhoundOffset&&(d=this.greyhoundOffset,f+='&offset=['+d.x+','+d.y+','+d.z+']');var g=Potree.XHRFactory.createXMLHttpRequest();g.open('GET',f,!0);var h=this;g.onreadystatechange=function(){if(4===g.readyState)if(200===g.status||0===g.status){var a=JSON.parse(g.responseText)||{},b=[];h.numPoints=a.n;c(a,h.name,b);a={};a[h.name]=h;for(var d=h.pcoGeometry,e=0;e<b.length;e++){var p=b[e].name,w=b[e].numPoints,
t=parseInt(p.charAt(p.length-1)),q=p.substring(0,p.length-1);q=a[q];var u=p.length-1;t=Potree.GreyhoundLoader.createChildAABB(q.boundingBox,t);t=new Potree.PointCloudGreyhoundGeometryNode(p,d,t,h.scale,h.offset);t.level=u;t.numPoints=w;t.hasChildren=0<b[e].children;t.spacing=d.spacing/Math.pow(2,u);q.addChild(t);a[p]=t}h.loadPoints()}else console.log('Failed to load file! HTTP status:',g.status,'file:',f)};try{g.send(null)}catch(l){console.log('fehler beim laden der punktwolke: '+l)}}};
...
var potree_nogui$classdecl$var19=function(a){var b=THREE.Object3D.call(this)||this;var c=new THREE.Texture;c.minFilter=THREE.LinearFilter;c.magFilter=THREE.LinearFilter;c=new THREE.SpriteMaterial({map:c,depthTest:!1,depthWrite:!1});b.material=c;b.sprite=new THREE.Sprite(c);b.add(b.sprite);b.borderThickness=4;b.fontface='Arial';b.fontsize=28;b.borderColor={r:0,g:0,b:0,a:1};b.backgroundColor={r:255,g:255,b:255,a:1};b.textColor={r:255,g:255,b:255,a:1};b.text='';b.setText(a);return b};
$jscomp.inherits(potree_nogui$classdecl$var19,THREE.Object3D);potree_nogui$classdecl$var19.prototype.setText=function(a){this.text!==a&&(this.text=a,this.update())};potree_nogui$classdecl$var19.prototype.setTextColor=function(a){this.textColor=a;this.update()};potree_nogui$classdecl$var19.prototype.setBorderColor=function(a){this.borderColor=a;this.update()};potree_nogui$classdecl$var19.prototype.setBackgroundColor=function(a){this.backgroundColor=a;this.update()};
potree_nogui$classdecl$var19.prototype.update=function(){var a=document.createElement('canvas'),b=a.getContext('2d');b.font='Bold '+this.fontsize+'px '+this.fontface;var c=b.measureText(this.text).width,d=10+c+2*this.borderThickness,e=1.4*this.fontsize+2*this.borderThickness;b.canvas.width=d;b.canvas.height=e;b.font='Bold '+this.fontsize+'px '+this.fontface;b.fillStyle='rgba('+this.backgroundColor.r+','+this.backgroundColor.g+','+this.backgroundColor.b+','+this.backgroundColor.a+')';b.strokeStyle=
...
var potree_nogui$classdecl$var20=function(){var a=THREE.Object3D.call(this)||this;a.constructor.counter=void 0===a.constructor.counter?0:a.constructor.counter+1;a.name='Measure_'+a.constructor.counter;a.points=[];a._showDistances=!0;a._showCoordinates=!1;a._showArea=!1;a._closed=!0;a._showAngles=!1;a._showHeight=!1;a.maxMarkers=Number.MAX_SAFE_INTEGER;a.sphereGeometry=new THREE.SphereGeometry(.4,10,10);a.color=new THREE.Color(16711680);a.lengthUnit={code:'m'};a.spheres=[];a.edges=[];a.sphereLabels=
...
potree_nogui$classdecl$var20.prototype.getAngleBetweenLines=function(a,b,c){b=(new THREE.Vector3).subVectors(b.position,a.position);a=(new THREE.Vector3).subVectors(c.position,a.position);return b.angleTo(a)};potree_nogui$classdecl$var20.prototype.getAngle=function(a){return 3>this.points.length||a>=this.points.length?0:this.getAngleBetweenLines(this.points[a],0===a?this.points[this.points.length-1]:this.points[a-1],this.points[(a+1)%this.points.length])};
...
potree_nogui$classdecl$var20.prototype.raycast=function(a,b){for(var c=0;c<this.points.length;c++)this.spheres[c].raycast(a,b);for(c=0;c<b.length;c++){var d=b[c];d.distance=a.ray.origin.distanceTo(d.point)}b.sort(function(a,b){return a.distance-b.distance})};
$jscomp.global.Object.defineProperties(potree_nogui$classdecl$var20.prototype,{showCoordinates:{configurable:!0,enumerable:!0,get:function(){return this._showCoordinates},set:function(a){this._showCoordinates=a;this.update()}},showAngles:{configurable:!0,enumerable:!0,get:function(){return this._showAngles},set:function(a){this._showAngles=a;this.update()}},showHeight:{configurable:!0,enumerable:!0,get:function(){return this._showHeight},set:function(a){this._showHeight=a;this.update()}},showArea:{configurable:!0,
enumerable:!0,get:function(){return this._showArea},set:function(a){this._showArea=a;this.update()}},closed:{configurable:!0,enumerable:!0,get:function(){return this._closed},set:function(a){this._closed=a;this.update()}},showDistances:{configurable:!0,enumerable:!0,get:function(){return this._showDistances},set:function(a){this._showDistances=a;this.update()}}});Potree.Measure=potree_nogui$classdecl$var20;
...
var potree_nogui$classdecl$var21=function(a){a=void 0===a?{}:a;a=Potree.Volume.call(this,a)||this;a.constructor.counter=void 0===a.constructor.counter?0:a.constructor.counter+1;a.name='box_'+a.constructor.counter;var b=new THREE.BoxGeometry(1,1,1);b.computeBoundingBox();var c=new THREE.Geometry;c.vertices.push(new THREE.Vector3(-.5,-.5,.5));c.vertices.push(new THREE.Vector3(.5,-.5,.5));c.vertices.push(new THREE.Vector3(.5,-.5,.5));c.vertices.push(new THREE.Vector3(.5,-.5,-.5));c.vertices.push(new THREE.Vector3(.5,
-.5,-.5));c.vertices.push(new THREE.Vector3(-.5,-.5,-.5));c.vertices.push(new THREE.Vector3(-.5,-.5,-.5));c.vertices.push(new THREE.Vector3(-.5,-.5,.5));c.vertices.push(new THREE.Vector3(-.5,.5,.5));c.vertices.push(new THREE.Vector3(.5,.5,.5));c.vertices.push(new THREE.Vector3(.5,.5,.5));c.vertices.push(new THREE.Vector3(.5,.5,-.5));c.vertices.push(new THREE.Vector3(.5,.5,-.5));c.vertices.push(new THREE.Vector3(-.5,.5,-.5));c.vertices.push(new THREE.Vector3(-.5,.5,-.5));c.vertices.push(new THREE.Vector3(-.5,
...
a.material);a.box.geometry.computeBoundingBox();a.boundingBox=a.box.geometry.boundingBox;a.add(a.box);a.frame=new THREE.LineSegments(c,new THREE.LineBasicMaterial({color:0}));a.add(a.frame);a.update();return a};$jscomp.inherits(potree_nogui$classdecl$var21,Potree.Volume);
potree_nogui$classdecl$var21.prototype.update=function(){this.boundingBox=this.box.geometry.boundingBox;this.boundingSphere=this.boundingBox.getBoundingSphere();this._clip?(this.box.visible=!1,this.label.visible=!1):(this.box.visible=!0,this.label.visible=this.showVolumeLabel)};potree_nogui$classdecl$var21.prototype.raycast=function(a,b){var c=[];this.box.raycast(a,c);0<c.length&&(a=c[0],b.push({distance:a.distance,object:this,point:a.point.clone()}))};
potree_nogui$classdecl$var21.prototype.getVolume=function(){return Math.abs(this.scale.x*this.scale.y*this.scale.z)};Potree.BoxVolume=potree_nogui$classdecl$var21;
var potree_nogui$classdecl$var22=function(a){a=void 0===a?{}:a;a=Potree.Volume.call(this,a)||this;a.constructor.counter=void 0===a.constructor.counter?0:a.constructor.counter+1;a.name='sphere_'+a.constructor.counter;var b=new THREE.SphereGeometry(1,32,32);b.computeBoundingBox();a.material=new THREE.MeshBasicMaterial({color:65280,transparent:!0,opacity:.3,depthTest:!0,depthWrite:!1});a.sphere=new THREE.Mesh(b,a.material);a.sphere.visible=!1;a.sphere.geometry.computeBoundingBox();a.boundingBox=a.sphere.geometry.boundingBox;
...
k=Math.sqrt(1-f*f),m=new THREE.Vector3(Math.cos(m)*k,Math.sin(m)*k,f),c.vertices.push(m),g=new THREE.Vector3(g*k,h*k,f),c.vertices.push(g);a.frame=new THREE.LineSegments(c,new THREE.LineBasicMaterial({color:0}));a.add(a.frame);c=new THREE.MeshBasicMaterial({wireframe:!0,color:0});a.frame=new THREE.Mesh(b,c);a.update();return a};$jscomp.inherits(potree_nogui$classdecl$var22,Potree.Volume);
potree_nogui$classdecl$var22.prototype.update=function(){this.boundingBox=this.sphere.geometry.boundingBox;this.boundingSphere=this.boundingBox.getBoundingSphere()};potree_nogui$classdecl$var22.prototype.raycast=function(a,b){var c=[];this.sphere.raycast(a,c);0<c.length&&(a=c[0],b.push({distance:a.distance,object:this,point:a.point.clone()}))};potree_nogui$classdecl$var22.prototype.getVolume=function(){return 4/3*Math.PI*this.scale.x*this.scale.y*this.scale.z};Potree.SphereVolume=potree_nogui$classdecl$var22;
...
potree_nogui$classdecl$var27.toString=function(a){a instanceof Array||(a=[a]);a=a.filter(function(a){return a instanceof Potree.Measure});var b=[];a=$jscomp.makeIterator(a);for(var c=a.next();!c.done;c=a.next())c=Potree.GeoJSONExporter.measurementToFeatures(c.value),b=b.concat(c);return JSON.stringify({type:'FeatureCollection',features:b},null,'\t')};Potree.GeoJSONExporter=potree_nogui$classdecl$var27;var potree_nogui$classdecl$var28=function(){};
potree_nogui$classdecl$var28.measurementPointSection=function(a){return(a=a.points[0].position)?'0\nCIRCLE\n8\nlayer_point\n10\n'+a.x+'\n20\n'+a.y+'\n30\n'+a.z+'\n40\n1.0\n':''};
potree_nogui$classdecl$var28.measurementPolylineSection=function(a){var b=8;a.closed&&(b+=1);b='0\nPOLYLINE\n8\nlayer_polyline\n62\n1\n66\n1\n10\n0.0\n20\n0.0\n30\n0.0\n70\n'+b+'\n';a=$jscomp.makeIterator(a.points);for(var c=a.next();!c.done;c=a.next())c=c.value,c=c.position,b+='0\nVERTEX\n8\n0\n10\n'+c.x+'\n20\n'+c.y+'\n30\n'+c.z+'\n70\n32\n';return b+'0\nSEQEND\n'};
potree_nogui$classdecl$var28.measurementSection=function(a){if(0===a.points.length)return'';if(1===a.points.length)return Potree.DXFExporter.measurementPointSection(a);if(2<=a.points.length)return Potree.DXFExporter.measurementPolylineSection(a)};
potree_nogui$classdecl$var28.toString=function(a){a instanceof Array||(a=[a]);a=a.filter(function(a){return a instanceof Potree.Measure});var b=a.filter(function(a){return a instanceof Potree.Measure}).map(function(a){return a.points}).reduce(function(a,b){return a.concat(b)}).map(function(a){return a.position}),c=new THREE.Vector3(Infinity,Infinity,Infinity),d=new THREE.Vector3(-Infinity,-Infinity,-Infinity);b=$jscomp.makeIterator(b);for(var e=b.next();!e.done;e=b.next())e=e.value,c.min(e),d.max(e);
c='999\nDXF created from potree\n0\nSECTION\n2\nHEADER\n9\n$ACADVER\n1\nAC1006\n9\n$INSBASE\n10\n0.0\n20\n0.0\n30\n0.0\n9\n$EXTMIN\n10\n'+c.x+'\n20\n'+c.y+'\n30\n'+c.z+'\n9\n$EXTMAX\n10\n'+d.x+'\n20\n'+d.y+'\n30\n'+d.z+'\n0\nENDSEC\n';d='0\nSECTION\n2\nENTITIES\n';a=$jscomp.makeIterator(a);for(b=a.next();!b.done;b=a.next())d+=Potree.DXFExporter.measurementSection(b.value);return c+(d+'0\nENDSEC\n')+'0\nEOF'};Potree.DXFExporter=potree_nogui$classdecl$var28;var potree_nogui$classdecl$var29=function(){};
potree_nogui$classdecl$var29.toString=function(a){var b='',c=Object.keys(a.data).filter(function(a){return'normal'!==a}).sort(function(a,b){if('position'===a)return-1;if('position'===b)return 1;if('color'===a)return-1;if('color'===b)return 1});b=[];for(var d=$jscomp.makeIterator(c),e=d.next();!e.done;e=d.next()){var f=e.value,g=a.data[f].length/a.numPoints;if('position'===f)b=b.concat(['x','y','z']);else if('color'===f)b=b.concat(['r','g','b','a']);else if(1<g)for(e=0;e<g;e++)b.push(f+'_'+e);else b.push(f)}b=
b.join(', ')+'\n';for(d=0;d<a.numPoints;d++){f=[];g=$jscomp.makeIterator(c);for(e=g.next();!e.done;e=g.next()){e=e.value;var h=a.data[e].length/a.numPoints;e=a.data[e].subarray(h*d,h*d+h).join(', ');f.push(e)}b+=f.join(', ')+'\n'}return b};Potree.CSVExporter=potree_nogui$classdecl$var29;var potree_nogui$classdecl$var30=function(){};
potree_nogui$classdecl$var30.toLAS=function(a){var b=a.boundingBox,c=b.min.clone(),d=b.min.distanceTo(b.max),e=new THREE.Vector3(.001,.001,.001);e=1E6<d?new THREE.Vector3(.01,.01,.01):new THREE.Vector3(.001,.001,.001);var f=function(a,b,c){c=new Uint8Array(c);for(var d=0;d<a.length;d++){var e=a.charCodeAt(d);c[b+d]=e}};d=new ArrayBuffer(227+28*a.numPoints);var g=new DataView(d),h=new Uint8Array(d);f('LASF',0,d);h[24]=1;h[25]=2;f('Potree 1.5',58,d);g.setUint16(94,227,!0);g.setUint32(96,227,!0);h[104]=
...
22,255*a.data.color[4*f+1],!0),g.setUint16(b+24,255*a.data.color[4*f+2],!0));b+=28}return d};Potree.LASExporter=potree_nogui$classdecl$var30;var potree_nogui$classdecl$var31=function(){Potree.PointCloudTreeNode.call(this);this.kdtree=this.sceneNode=this.right=this.left=null};$jscomp.inherits(potree_nogui$classdecl$var31,Potree.PointCloudTreeNode);potree_nogui$classdecl$var31.prototype.getNumPoints=function(){return this.geometryNode.numPoints};potree_nogui$classdecl$var31.prototype.isLoaded=function(){return!0};
potree_nogui$classdecl$var31.prototype.isTreeNode=function(){return!0};potree_nogui$classdecl$var31.prototype.isGeometryNode=function(){return!1};potree_nogui$classdecl$var31.prototype.getLevel=function(){return this.geometryNode.level};potree_nogui$classdecl$var31.prototype.getBoundingSphere=function(){return this.geometryNode.boundingSphere};potree_nogui$classdecl$var31.prototype.getBoundingBox=function(){return this.geometryNode.boundingBox};
potree_nogui$classdecl$var31.prototype.toTreeNode=function(a){var b=null;this.left===a?b=this.left:this.right===a&&(b=this.right);if(b.loaded){a=new Potree.PointCloudArena4DNode;var c=THREE.PointCloud(b.geometry,this.kdtree.material);c.visible=!1;a.kdtree=this.kdtree;a.geometryNode=b;a.sceneNode=c;a.parent=this;a.left=this.geometryNode.left;a.right=this.geometryNode.right}};
potree_nogui$classdecl$var31.prototype.getChildren=function(){var a=[];this.left&&a.push(this.left);this.right&&a.push(this.right);return a};Potree.PointCloudArena4DNode=potree_nogui$classdecl$var31;
var potree_nogui$classdecl$var32=function(a){var b=Potree.PointCloudTree.call(this)||this;b.root=null;a.root?b.root=a.root:a.addEventListener('hierarchy_loaded',function(){b.root=a.root});b.visiblePointsTarget=2E6;b.minimumNodePixelSize=150;b.position.sub(a.offset);b.updateMatrix();b.numVisibleNodes=0;b.numVisiblePoints=0;b.boundingBoxNodes=[];b.loadQueue=[];b.visibleNodes=[];b.pcoGeometry=a;b.boundingBox=b.pcoGeometry.boundingBox;b.boundingSphere=b.pcoGeometry.boundingSphere;b.material=new Potree.PointCloudMaterial({vertexColors:THREE.VertexColors,
size:.05,treeType:Potree.TreeType.KDTREE});b.material.sizeType=Potree.PointSizeType.ATTENUATED;b.material.size=.05;b.profileRequests=[];b.name='';return b};$jscomp.inherits(potree_nogui$classdecl$var32,Potree.PointCloudTree);potree_nogui$classdecl$var32.prototype.getBoundingBoxWorld=function(){this.updateMatrixWorld(!0);return Potree.utils.computeTransformedBoundingBox(this.boundingBox,this.matrixWorld)};
potree_nogui$classdecl$var32.prototype.setName=function(a){this.name!==a&&(this.name=a,this.dispatchEvent({type:'name_changed',name:a,pointcloud:this}))};potree_nogui$classdecl$var32.prototype.getName=function(){return this.name};potree_nogui$classdecl$var32.prototype.getLevel=function(){return this.level};
potree_nogui$classdecl$var32.prototype.toTreeNode=function(a,b){var c=this,d=new Potree.PointCloudArena4DNode,e=new THREE.Points(a.geometry,this.material);e.frustumCulled=!1;e.onBeforeRender=function(b,e,h,l,k,m){k.program&&(b.getContext().useProgram(k.program.program),k.program.getUniforms().map.level&&(e=a.getLevel(),k.uniforms.level.value=e,k.program.getUniforms().map.level.setValue(b.getContext(),e)),c.visibleNodeTextureOffsets&&k.program.getUniforms().map.vnStart&&(e=c.visibleNodeTextureOffsets.get(d),
...
b.left===d?b.left=a:b.right===d&&(b.right=a)});return d};potree_nogui$classdecl$var32.prototype.updateMaterial=function(a,b,c,d){a.fov=Math.PI/180*c.fov;a.screenWidth=d.domElement.clientWidth;a.screenHeight=d.domElement.clientHeight;a.spacing=this.pcoGeometry.spacing;a.near=c.near;a.far=c.far;this.maxLevel>a.levels&&(a.levels=this.maxLevel+2);b=this.boundingBox.getSize();a.bbSize=[b.x,b.y,b.z]};potree_nogui$classdecl$var32.prototype.updateVisibleBounds=function(){};
potree_nogui$classdecl$var32.prototype.hideDescendants=function(a){for(var b=[],c=0;c<a.children.length;c++){var d=a.children[c];d.visible&&b.push(d)}for(;0<b.length;)for(a=b.shift(),a.visible=!1,a.boundingBoxNode&&(a.boundingBoxNode.visible=!1),c=0;c<a.children.length;c++)d=a.children[c],d.visible&&b.push(d)};
potree_nogui$classdecl$var32.prototype.updateMatrixWorld=function(a){!0===this.matrixAutoUpdate&&this.updateMatrix();if(!0===this.matrixWorldNeedsUpdate||!0===a)void 0===this.parent?this.matrixWorld.copy(this.matrix):this.matrixWorld.multiplyMatrices(this.parent.matrixWorld,this.matrix),this.matrixWorldNeedsUpdate=!1};
potree_nogui$classdecl$var32.prototype.nodesOnRay=function(a,b){var c=[];b=b.clone();for(var d=0;d<a.length;d++){var e=a[d],f=e.getBoundingSphere().clone().applyMatrix4(e.sceneNode.matrixWorld);b.intersectsSphere(f)&&c.push(e)}return c};
potree_nogui$classdecl$var32.prototype.pick=function(a,b,c,d){d=void 0===d?{}:d;var e=a.renderer,f=a.pRenderer;performance.mark('pick-start');var g=function(a,b){return void 0!==a?a:b};a=g(d.pickWindowSize,17);g(d.pickOutsideClipRegion,!1);var h=e.getSize(),l=Math.ceil(g(d.width,h.width));h=Math.ceil(g(d.height,h.height));var k=g(d.pointSizeType,this.material.pointSizeType);g=g(d.pointSize,this.material.size);c=this.nodesOnRay(this.visibleNodes,c);if(0===c.length)return null;if(!this.pickState){var m=
new THREE.Scene,n=new Potree.PointCloudMaterial;n.pointColorType=Potree.PointColorType.POINT_INDEX;this.pickState={renderTarget:new THREE.WebGLRenderTarget(1,1,{minFilter:THREE.LinearFilter,magFilter:THREE.NearestFilter,format:THREE.RGBAFormat}),material:n,scene:m}}m=this.pickState;n=m.material;n.pointSizeType=k;n.shape=this.material.shape;n.size=g;n.uniforms.minSize.value=this.material.uniforms.minSize.value;n.uniforms.maxSize.value=this.material.uniforms.maxSize.value;n.classification=this.material.classification;
d.pickClipped?(n.clipBoxes=this.material.clipBoxes,n.clipTask=this.material.clipTask===Potree.ClipTask.HIGHLIGHT?Potree.ClipTask.NONE:this.material.clipTask):n.clipBoxes=[];this.updateMaterial(n,c,b,e);m.renderTarget.setSize(l,h);g=new THREE.Vector2(d.x,d.y);k=e.getContext();k.enable(k.SCISSOR_TEST);k.scissor(parseInt(g.x-(a-1)/2),parseInt(g.y-(a-1)/2),parseInt(a),parseInt(a));e.state.buffers.depth.setTest(n.depthTest);e.state.buffers.depth.setMask(n.depthWrite);e.state.setBlending(THREE.NoBlending);
e.clearTarget(m.renderTarget,!0,!0,!0);e.setRenderTarget(m.renderTarget);k.clearColor(0,0,0,0);e.clearTarget(m.renderTarget,!0,!0,!0);var p=this.material;this.material=n;f.renderOctree(this,c,b,m.renderTarget);this.material=p;b=parseInt(Math.min(Math.max(0,g.x-(a-1)/2),l));f=parseInt(Math.min(Math.max(0,g.y-(a-1)/2),h));l=parseInt(Math.min(b+a,l)-b);h=parseInt(Math.min(f+a,h)-f);l=new Uint8Array(4*l*h);k.readPixels(b,f,a,a,k.RGBA,k.UNSIGNED_BYTE,l);e.setRenderTarget(null);e.resetGLState();e.setScissorTest(!1);
...
'computeVisibilityTextureData-end'));return{data:b,offsets:c}};$jscomp.global.Object.defineProperties(potree_nogui$classdecl$var32.prototype,{progress:{configurable:!0,enumerable:!0,get:function(){return this.pcoGeometry.root?0<Potree.numNodesLoading?0:1:0}}});Potree.PointCloudArena4D=potree_nogui$classdecl$var32;
var potree_nogui$classdecl$var33=function(){this.pcoGeometry=this.number=this.boundingBox=this.right=this.left=null;this.loaded=!1;this.level=this.numPoints=0;this.children=[];this.oneTimeDisposeHandlers=[]};potree_nogui$classdecl$var33.prototype.isGeometryNode=function(){return!0};potree_nogui$classdecl$var33.prototype.isTreeNode=function(){return!1};potree_nogui$classdecl$var33.prototype.isLoaded=function(){return this.loaded};potree_nogui$classdecl$var33.prototype.getBoundingSphere=function(){return this.boundingSphere};
potree_nogui$classdecl$var33.prototype.getBoundingBox=function(){return this.boundingBox};potree_nogui$classdecl$var33.prototype.getChildren=function(){var a=[];this.left&&a.push(this.left);this.right&&a.push(this.right);return a};potree_nogui$classdecl$var33.prototype.getLevel=function(){return this.level};
potree_nogui$classdecl$var33.prototype.load=function(){if(!(this.loaded||this.loading||Potree.numNodesLoading>=Potree.maxNodesLoading)){this.loading=!0;Potree.numNodesLoading++;var a=this.pcoGeometry.url+'?node='+this.number,b=Potree.XHRFactory.createXMLHttpRequest();b.open('GET',a,!0);b.responseType='arraybuffer';var c=this;b.onreadystatechange=function(){if(4===b.readyState&&200===b.status){var a=b.response,e=new DataView(a);a=a.byteLength/17;var f=new ArrayBuffer(28*a);new DataView(f);f=new Float32Array(3*
a);for(var g=new Uint8Array(4*a),h=new Float32Array(a),l=new Uint8Array(a),k=new ArrayBuffer(4*a),m=new Uint32Array(k),n=new THREE.Box3,p=0;p<a;p++){var w=e.getFloat32(17*p,!0)+c.boundingBox.min.x,t=e.getFloat32(17*p+4,!0)+c.boundingBox.min.y,q=e.getFloat32(17*p+8,!0)+c.boundingBox.min.z,u=e.getUint8(17*p+12,!0),z=e.getUint8(17*p+13,!0),A=e.getUint8(17*p+14,!0),v=e.getUint8(17*p+15,!0),r=e.getUint8(17*p+16,!0);n.expandByPoint(new THREE.Vector3(w,t,q));f[3*p]=w;f[3*p+1]=t;f[3*p+2]=q;g[4*p]=u;g[4*p+
1]=z;g[4*p+2]=A;g[4*p+3]=255;h[p]=v;l[p]=r;m[p]=p}e=new THREE.BufferGeometry;e.addAttribute('position',new THREE.BufferAttribute(f,3));e.addAttribute('color',new THREE.BufferAttribute(g,4,!0));e.addAttribute('intensity',new THREE.BufferAttribute(h,1));e.addAttribute('classification',new THREE.BufferAttribute(l,1));f=new THREE.BufferAttribute(new Uint8Array(k),4,!0);e.addAttribute('indices',f);c.geometry=e;c.numPoints=a;c.loaded=!0;c.loading=!1;Potree.numNodesLoading--}};b.send(null)}};
potree_nogui$classdecl$var33.prototype.dispose=function(){if(this.geometry&&null!=this.parent){this.geometry.dispose();this.geometry=null;this.loaded=!1;for(var a=0;a<this.oneTimeDisposeHandlers.length;a++)(0,this.oneTimeDisposeHandlers[a])();this.oneTimeDisposeHandlers=[]}};potree_nogui$classdecl$var33.prototype.getNumPoints=function(){return this.numPoints};Potree.PointCloudArena4DGeometryNode=potree_nogui$classdecl$var33;
var potree_nogui$classdecl$var34=function(){var a=THREE.EventDispatcher.call(this)||this;a.numPoints=0;a.version=0;a.boundingBox=null;a.numNodes=0;a.name=null;a.provider=null;a.url=null;a.root=null;a.levels=0;a._spacing=null;a.pointAttributes=new Potree.PointAttributes(['POSITION_CARTESIAN','COLOR_PACKED']);return a};$jscomp.inherits(potree_nogui$classdecl$var34,THREE.EventDispatcher);
potree_nogui$classdecl$var34.load=function(a,b){var c=Potree.XHRFactory.createXMLHttpRequest();c.open('GET',a+'?info',!0);c.onreadystatechange=function(){try{if(4===c.readyState&&200===c.status){var d=JSON.parse(c.responseText),e=new Potree.PointCloudArena4DGeometry;e.url=a;e.name=d.Name;e.provider=d.Provider;e.numNodes=d.Nodes;e.numPoints=d.Points;e.version=d.Version;e.boundingBox=new THREE.Box3((new THREE.Vector3).fromArray(d.BoundingBox.slice(0,3)),(new THREE.Vector3).fromArray(d.BoundingBox.slice(3,
...
potree_nogui$classdecl$var34.prototype.loadHierarchy=function(){var a=this,b=this.url+'?tree',c=Potree.XHRFactory.createXMLHttpRequest();c.open('GET',b,!0);c.responseType='arraybuffer';c.onreadystatechange=function(){if(4===c.readyState&&200===c.status){var b=c.response,e=b.byteLength/3;b=new DataView(b);for(var f=[],g=null,h=0,l=0;l<e;l++){var k=b.getUint8(3*l,!0),m=0<(k&1),n=0<(k&2),p=0<(k&8),w=0<(k&16),t=null;0<(k&4)?t='X':p&&(t='Y');w&&(t='Z');k=new Potree.PointCloudArena4DGeometryNode;k.hasLeft=
m;k.hasRight=n;k.split=t;k.isLeaf=!m&&!n;k.number=l;k.left=null;k.right=null;k.pcoGeometry=a;k.level=f.length;h=Math.max(h,k.level);0<f.length?(m=f[f.length-1],k.boundingBox=m.boundingBox.clone(),n=m.boundingBox.getSize(),m.hasLeft&&!m.left?(m.left=k,m.children.push(k),'X'===m.split?k.boundingBox.max.x=k.boundingBox.min.x+n.x/2:'Y'===m.split?k.boundingBox.max.y=k.boundingBox.min.y+n.y/2:'Z'===m.split&&(k.boundingBox.max.z=k.boundingBox.min.z+n.z/2),m=k.boundingBox.getCenter(),n=k.boundingBox.getSize().length()/
2,k.boundingSphere=new THREE.Sphere(m,n)):(m.right=k,m.children.push(k),'X'===m.split?k.boundingBox.min.x+=n.x/2:'Y'===m.split?k.boundingBox.min.y+=n.y/2:'Z'===m.split&&(k.boundingBox.min.z+=n.z/2),m=k.boundingBox.getCenter(),n=k.boundingBox.getSize().length()/2,k.boundingSphere=new THREE.Sphere(m,n))):(g=k,g.boundingBox=a.boundingBox.clone(),m=g.boundingBox.getCenter(),n=g.boundingBox.getSize().length()/2,g.boundingSphere=new THREE.Sphere(m,n));m=k.boundingBox.getSize();k.spacing=(m.x+m.y+m.z)/3/
75;k.estimatedSpacing=k.spacing;f.push(k);if(k.isLeaf)for(k=!1;!k&&0<f.length;)f.pop(),k=f[f.length-1],k=0<f.length&&k.hasRight&&null==k.right}a.root=g;a.levels=h;a.dispatchEvent({type:'hierarchy_loaded'})}};c.send(null)};$jscomp.global.Object.defineProperties(potree_nogui$classdecl$var34.prototype,{spacing:{configurable:!0,enumerable:!0,get:function(){if(this._spacing)return this._spacing;if(this.root)return this.root.spacing},set:function(a){this._spacing=a}}});
Potree.PointCloudArena4DGeometry=potree_nogui$classdecl$var34;var potree_nogui$classdecl$var35=function(a,b){this.gl=a;this.material=b;this.program=a.createProgram();this.recompile()};potree_nogui$classdecl$var35.prototype.compileShader=function(a,b){var c=this.gl;a=c.createShader(a);c.shaderSource(a,b);c.compileShader(a);return c.getShaderParameter(a,c.COMPILE_STATUS)?a:(console.error('could not compile shader:'),c=c.getShaderInfoLog(a),console.error(c,b),null)};
potree_nogui$classdecl$var35.prototype.recompile=function(){var a=this.gl,b=this.compileShader(a.VERTEX_SHADER,this.material.vertexShader),c=this.compileShader(a.FRAGMENT_SHADER,this.material.fragmentShader);if(null!==b&&null!==c){var d=this.program;a.attachShader(d,b);a.attachShader(d,c);a.linkProgram(d);if(a.getProgramParameter(d,a.LINK_STATUS)){a.detachShader(d,b);a.detachShader(d,c);a.deleteShader(b);a.deleteShader(c);a.useProgram(d);b={};c=a.getProgramParameter(d,a.ACTIVE_UNIFORMS);for(var e=
0;e<c;e++){var f=a.getActiveUniform(d,e).name,g=a.getUniformLocation(d,f);b[f]=g}this.uniforms=b}else console.error('could not compile/link program:'),console.error(this.material.vertexShader),console.error(this.material.fragmentShader)}};Potree.GLProgram=potree_nogui$classdecl$var35;var potree_nogui$classdecl$var36=function(a,b,c,d,e){this.name=a;this.bytes=b;this.numElements=c;this.normalized=e;this.type=d};Potree.InterleavedBufferAttribute=potree_nogui$classdecl$var36;
var potree_nogui$classdecl$var37=function(a,b,c){this.data=a;this.attributes=b;this.stride=b.reduce(function(a,b){return a+b.bytes},0);this.stride=4*Math.ceil(this.stride/4);this.numElements=c};potree_nogui$classdecl$var37.prototype.offset=function(a){for(var b=0,c=$jscomp.makeIterator(this.attributes),d=c.next();!d.done;d=c.next()){d=d.value;if(d.name===a)return b;b+=d.bytes}return null};Potree.InterleavedBuffer=potree_nogui$classdecl$var37;
...
a.readUInt8(2))throw Error('not a valid tiff file');b=a.readUInt32LE(4);console.log('offsetToFirstIFD',b);var c=[],k=b;for(b=0;100>b;){console.log('currentIFDOffset',k);var m=a.readUInt16LE(k),q=a.readUInt32LE(k+2+12*m);console.log('next offset: ',k+2+12*m);k=a.slice(k+2,k+2+12*m);for(var u=0;u<m;u++){var z=f.fromValue(k.readUInt16LE(12*u)),A=e.fromValue(k.readUInt16LE(12*u+2)),v=k.readUInt32LE(12*u+4),r=k.readUInt32LE(12*u+8),x=A.bytes*v,y=void 0;4>=x?y=r:(y=new Uint8Array(x),y.set(a.slice(r,r+x)),
y=new (g.get(A))(y.buffer),A===e.ASCII&&(y=String.fromCharCode.apply(String,[].concat($jscomp.arrayFromIterable(y)))));z=new h(z,A,v,r,y);c.push(z)}console.log('nextIFDOffset',q);if(0===q)break;k=q;b++}k=function(a){for(var b=$jscomp.makeIterator(c),d=b.next();!d.done;d=b.next())if(d=d.value,d.tag===a)return d;return null};b=k(f.IMAGE_WIDTH,c).value;m=k(f.IMAGE_HEIGHT,c).value;k(f.COMPRESSION,c);q=k(f.ROWS_PER_STRIP,c).value;A=k(f.STRIP_OFFSETS,c);z=k(f.STRIP_BYTE_COUNTS,c);k=Math.ceil(m/q);u=[];
for(v=0;v<z.count;v++)r=z.type,x=z.offset+v*r.bytes,y=void 0,r===e.SHORT?y=a.readUInt16LE(x):r===e.LONG&&(y=a.readUInt32LE(x)),u.push(y);z=[];for(v=0;v<A.count;v++)r=A.type,x=A.offset+v*r.bytes,y=void 0,r===e.SHORT?y=a.readUInt16LE(x):r===e.LONG&&(y=a.readUInt32LE(x)),z.push(y);A=new Uint8Array(b*m*3);for(r=v=0;r<k;r++){x=z[r];x=a.slice(x,x+u[r]);y=3*b;for(var B=0;B<q;B++){var C=x.slice(B*y,B*y+y);A.set(C,v*y);if(C.length===y)v++;else break}}console.log('width: '+b);console.log('height: '+m);console.log('numStrips: '+
k);console.log('stripByteCounts',u.join(', '));console.log('stripOffsets',z.join(', '));a=new l;a.width=b;a.height=m;a.buffer=A;a.metadata=c;return a};var k=function(){};k.toTiffBuffer=function(a,b){b=void 0===b?{}:b;var c=new Uint8Array([73,73,42,0,8,0,0,0]),d=$jscomp.makeIterator([a.width,a.height]),g=d.next().value;d=d.next().value;var k=[new h(f.IMAGE_WIDTH,e.SHORT,1,null,g),new h(f.IMAGE_HEIGHT,e.SHORT,1,null,d),new h(f.BITS_PER_SAMPLE,e.SHORT,4,null,new Uint16Array([8,8,8,8])),new h(f.COMPRESSION,
...
1,null,new Uint32Array([1,1]))];b.ifdEntries&&k.push.apply(k,[].concat($jscomp.arrayFromIterable(b.ifdEntries)));var l=12*k.length+14,m=new Map;b=new Map;for(var n=$jscomp.makeIterator(k),v=n.next();!v.done;v=n.next()){v=v.value;var r=new ArrayBuffer(12),x=new DataView(r);x.setUint16(0,v.tag.value,!0);x.setUint16(2,v.type.value,!0);x.setUint32(4,v.count,!0);1===v.count&&4>=v.type.bytes?x.setUint32(8,v.value,!0):(x.setUint32(8,l,!0),x=new Uint8Array(v.count*v.type.bytes),v.type===e.ASCII?x.set(new Uint8Array(v.value.split('').map(function(a){return a.charCodeAt(0)}))):
x.set(new Uint8Array(v.value.buffer)),b.set(v.tag,x),l+=x.byteLength);m.set(v.tag,r)}(new DataView(m.get(f.STRIP_OFFSETS))).setUint32(8,l,!0);l=function(a){var b=a.reduce(function(a,b){return a+b.byteLength},0);b=new Uint8Array(b);var c=0;a=$jscomp.makeIterator(a);for(var d=a.next();!d.done;d=a.next())d=d.value,b.set(new Uint8Array(d),c),c+=d.byteLength;return b};k=l([].concat([new Uint16Array([k.length])],$jscomp.arrayFromIterable(m.values()),[new Uint32Array([0])]));b=l([].concat($jscomp.arrayFromIterable(b.values())));
